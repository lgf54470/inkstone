...
import { addTagToFrontMatter, deleteFrontMatterProperty, extractAttachmentIds, extractTags, interpolateNewNoteTemplate, mergeTagsIntoFrontMatter, parseFrontMatter, renameFrontMatterProperty, renderNewNoteTemplate, setFrontMatterProperty, upsertFrontMatterProperty } from './markdown-utils'
...
describe('extractAttachmentIds', () => {
  ...
  it('collects references inside an md-example nested in an md-example', () => {
    expect(extractAttachmentIds(
      `~~~~md-example\n\`\`\`md-example\n![a](/api/files/${idA})\n\`\`\`\n~~~~`,
    )).toEqual([idA])
  })
