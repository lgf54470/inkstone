export const messages = {
'music.hub_title': '音乐库',
'music.hub_sidebar': '音乐导航',
'music.hub_open_navigation': '打开音乐导航',
'music.hub_open_now_playing': '打开正在播放面板',
'music.open_hub': '打开音乐库',
'music.open_hub_track': '打开音乐库：{value0}',
'music.section_library': '曲库',
'music.section_playlists': '歌单',
'music.tracks': '歌曲',
'music.all_tracks': '全部歌曲',
'music.favorites': '我的收藏',
'music.pinned': '置顶歌曲',
'music.recently_played': '最近播放',
'music.albums': '专辑',
'music.artists': '歌手',
'music.duplicates': '重复文件',
'music.no_duplicates': '没有发现重复文件',
'music.duplicates_summary': '共 {value0} 组重复 · 多出 {value1} 份 · 约可释放 {value2}',
'music.group_back': '返回分组',
'music.group_track_count': '共 {value0} 首',
'music.group_detail_meta': '共 {value0} 首 · {value1}',
'music.loading': '正在加载音乐…',
'music.load_failed': '音乐库加载失败',
'music.retry': '重试',
'music.playback_slow': '这首歌的网络传输太慢，无法播放。请改用本地或 R2 曲目。',
'music.playback_repeated_failures': '连续多首无法播放，已停止自动跳过。请检查网络或音频文件。',
'music.no_tracks': '还没有歌曲',
'music.no_tracks_hint': '上传一个音频文件即可开始建立曲库。',
'music.no_favorites': '还没有收藏',
'music.no_pinned': '还没有置顶歌曲',
'music.no_results': '没有匹配的歌曲',
'music.open_lyrics': '打开完整歌词',
'music.no_lyrics': '这首歌没有歌词',
'music.lyrics_loading': '正在加载歌词',
'music.nothing_playing': '未在播放',
'music.now_playing': '正在播放',
'music.play': '播放',
'music.pause': '暂停',
'music.play_pause': '播放或暂停',
'music.previous': '上一首',
'music.next': '下一首',
'music.seek': '播放进度',
'music.volume': '音量',
'music.mute': '静音',
'music.unmute': '取消静音',
'music.mode_order': '顺序播放',
'music.mode_repeat_all': '列表循环',
'music.mode_repeat_one': '单曲循环',
'music.mode_shuffle': '随机播放',
'music.favorite': '收藏',
'music.unfavorite': '取消收藏',
'music.pin': '置顶',
'music.unpin': '取消置顶',
'music.favorite_added': '已收藏',
'music.favorite_removed': '已取消收藏',
'music.pin_added': '已置顶',
'music.pin_removed': '已取消置顶',
'music.action_failed': '操作失败',
'music.upload': '上传',
'music.transfer_title': '音乐传输',
'music.upload_target': '上传目标',
'music.transfers_uploads': '上传中',
'music.transfers_downloads': '下载到本机',
'music.download_progress': '正在下载 {value0}%',
'music.download_done': '已下载 {value0}',
'music.download_failed': '下载失败',
'music.download_selected': '下载所选 {value0} 首',
'music.make_offline': '供离线播放',
'music.remove_offline': '移出离线播放',
'music.make_offline_selected': '离线保存 {value0} 首',
'music.offline_saved': '已加入离线播放 {value0} 首',
'music.offline_removed': '已移出离线播放 {value0} 首',
'music.offline_save_failed': '离线保存失败',
'music.offline_quota_full': '离线存储空间已满',
'music.transfers_library_jobs': '批量任务',
'music.job_metadata': '扫描标签',
'music.job_covers': '在线匹配封面',
'music.job_progress': '已处理 {value0}/{value1} 首',
'music.job_failed': '任务已中断',
'music.upload_hint': '把音频或视频文件拖到这里，或从设备中选择文件；标题、歌手、专辑、封面与歌词会自动读取文件内标签。',
'music.upload_choose': '选择文件',
'music.upload_choose_folder': '选择文件夹',
'music.upload_progress': '正在上传 {value0}%',
'music.upload_done': '已上传 {value0} 首歌曲',
'music.upload_failed': '上传失败',
'music.upload_unsupported': '不支持的音频或视频格式',
'music.upload_too_large': '文件超出大小限制',
'music.upload_skipped_count': '已跳过 {value0} 个文件',
'music.upload_cancel': '取消上传',
'music.upload_dismiss': '关闭',
'music.delete_track': '删除歌曲',
'music.delete_track_confirm': '确定删除《{value0}》吗？音频文件会从存储和所有歌单中移除。',
'music.delete': '删除',
'music.deleted': '已删除',
'music.delete_failed': '删除失败',
'music.edit_track': '编辑歌曲信息',
'music.field_title': '标题',
'music.field_title_required': '标题不能为空',
'music.field_artist': '歌手',
'music.field_album': '专辑',
'music.field_lyric': '歌词（LRC）',
'music.save': '保存',
'music.saved': '已保存',
'music.save_failed': '保存失败',
'music.rename': '重命名',
'music.search_placeholder': '搜索歌曲、歌手、专辑或拼音',
'music.search_history': '搜索历史',
'music.search_clear_history': '清空历史',
'music.search_clear': '清除搜索',
'music.search_results': '“{value0}”的搜索结果',
'music.search_truncated': '共 {value1} 条匹配，仅显示前 {value0} 条——请缩小搜索范围',
'music.queue': '播放队列',
'music.queue_empty': '队列为空',
'music.queue_count': '队列中 {value0} 首',
'music.play_next': '下一首播放',
'music.add_to_queue': '添加到队列',
'music.clear_queue': '清空队列',
'music.clear_queue_confirm': '清空队列中的 {value0} 首歌曲？播放将停止。',
'music.added_to_queue': '已添加到队列',
'music.queue_added_count': '已加入队列 {value0} 首',
'music.import_m3u': '导入播放列表（M3U）',
'music.m3u_imported_partial': '{value0} 条中有 {value1} 条未匹配到曲库',
'music.m3u_imported_none': '该播放列表没有任何条目匹配到曲库',
'music.add_to_playlist': '添加到歌单',
'music.remove_from_playlist': '从歌单移除',
'music.move_up': '上移',
'music.move_down': '下移',
'music.added_to_playlist': '已添加到「{value0}」',
'music.already_in_playlist': '已在「{value0}」中',
'music.new_playlist': '新建歌单',
'music.edit_playlist': '编辑歌单',
'music.playlist_name': '歌单名称',
'music.playlist_description': '描述',
'music.playlist_created': '歌单已创建',
'music.delete_playlist': '删除歌单',
'music.delete_playlist_confirm': '确定删除歌单「{value0}」吗？其中的歌曲仍保留在曲库。',
'music.playlist_empty': '这个歌单还是空的',
'music.playlist_track_count': '{value0} 首歌曲',
'music.no_playlists': '还没有歌单',
'music.share_playlist': '分享歌单',
'music.unshare_playlist': '取消分享',
'music.share_link_copied': '分享链接已复制',
'music.share_link_copy_failed': '链接复制失败',
'music.unshared_playlist': '已取消歌单分享',
'music.shared_playlist': '分享的歌单',
'music.playlist_link_gone': '该歌单分享链接已失效或已被取消。',
'music.playlist_link_failed': '歌单加载失败。',
'music.tags': '标签',
'music.remove_tag': '从歌曲中移除标签',
'music.refresh_metadata': '扫描封面与歌词',
'music.metadata_force': '按标签强制覆盖元数据',
'music.metadata_force_confirm': '将重新扫描列表中的全部曲目，用文件里的标签覆盖已保存的封面、歌词、歌手与专辑。手动修改的内容会丢失。',
'music.metadata_refreshed': '已更新 {value0} 首',
'music.metadata_unavailable': '这些文件里没有可读取的封面或歌词',
'music.search_lyrics': '在线搜索歌词',
'music.search_lyrics_confirm': '要用在线搜索结果替换《{value0}》已保存的歌词吗？',
'music.replace': '替换',
'music.lyrics_matched': '歌词已更新',
'music.lyrics_unmatched': '未匹配到歌词',
'music.lyric_search_failed': '歌词搜索失败',
'music.match_covers': '联网补封面',
'music.covers_matched': '已匹配 {value0} 首封面',
'music.covers_unmatched': '没有在线匹配到封面',
'music.tag_created': '标签已创建',
'music.tag_exists': '标签“{value0}”已存在',
'music.tag_deleted': '标签已删除',
'music.no_tags': '还没有标签',
'music.track_tags': '这首歌的标签',
'music.add_tag': '添加标签',
'music.sort': '排序',
'music.sort_recent': '最近添加',
'music.sort_title': '标题',
'music.sort_artist': '歌手',
'music.sort_plays': '播放最多',
'music.view_list': '列表视图',
'music.view_grid': '网格视图',
'music.view_mode': '视图模式',
'music.stats_tracks': '歌曲',
'music.stats_duration': '总时长',
'music.stats_size': '占用空间',
'music.play_count': '播放次数',
'music.track_duration': '时长',
'music.unknown_artist': '未知歌手',
'music.unknown_album': '未知专辑',
'music.mini_player': '浮动播放器',
'music.show_mini_player': '显示浮动播放器',
'music.hide_mini_player': '隐藏浮动播放器',
'music.expand_player': '展开播放器',
'music.collapse_player': '收起播放器',
'music.lyrics': '歌词',
'music.loop_start': '标记循环起点（A）',
'music.loop_marker_a': 'A',
'music.loop_marker_b': 'B',
'music.loop_end': '标记循环终点（B）',
'music.loop_clear': '清除 A-B 循环',
'music.loop_range': '循环 {value0} 至 {value1}',
'music.lyric_offset_earlier': '歌词提前四分之一秒',
'music.lyric_offset_later': '歌词延后四分之一秒',
'music.lyric_offset_reset': '重置歌词校准',
'music.lyric_offset_state': '歌词偏移 {value0}',
'music.details': '详情',
'music.source': '来源',
'music.file_size': '文件大小',
'music.added_at': '添加时间',
'music.keyboard_hint': '空格播放或暂停，Alt+方向键快退快进十秒，Ctrl/⌘+方向键切换上一首/下一首',
'music.select_all': '全选',
'music.clear_selection': '取消选择',
'music.selected_count': '已选择 {value0} 首',
'music.batch_favorite': '收藏',
'music.batch_unfavorite': '取消收藏',
'music.batch_pin': '置顶',
'music.batch_unpin': '取消置顶',
'music.batch_delete': '删除',
'music.batch_delete_confirm': '确定删除选中的 {value0} 首歌曲吗？',
'music.batch_done': '已更新 {value0} 首歌曲',
'music.batch_partial': '已更新 {value0} 首，另 {value1} 首失败仍保持选中',
'music.selection': '多选',
'music.selection_hint': '单击选中，Ctrl 点击多选，Shift 点击连选',
'music.invert_selection': '反选',
'music.select_all_tracks': '全选歌曲',
'music.move_to_tag': '移动到标签',
'music.moved_to_tag': '已移动 {value0} 首',
'music.open_menu': '更多操作',
'music.track_menu': '歌曲操作',
'music.playlist_menu': '歌单操作',
'music.download': '下载',
'music.shuffle_all': '随机播放全部',
'music.play_all': '播放全部',
'music.storage_quota': '已用空间',
'music.remove_from_queue': '从队列移除',
'music.source_all': '全部来源',
'music.source_r2': '云端 (R2)',
'music.source_webdav': 'WebDAV',
'music.table_index': '#',
'music.table_title': '标题',
'music.table_artist': '歌手',
'music.table_album': '专辑',
'music.table_duration': '时长',
'music.queue_search': '搜索播放队列',
'music.no_queue_match': '队列中没有匹配的歌曲',
'music.webdav_title': 'WebDAV 音乐',
'music.webdav_upload': '上传到 WebDAV',
'music.webdav_import': '添加到曲库',
'music.webdav_import_all': '添加全部音频',
'music.webdav_not_configured': '还没有配置 WebDAV 目标',
'music.webdav_configure_hint': '请在「设置 → 备份」中选择 WebDAV 目标。',
'music.webdav_open_settings': '打开备份设置',
'music.webdav_empty': '这个目录里没有音频文件',
'music.webdav_loading': '正在读取 WebDAV 目录…',
'music.webdav_failed': '读取 WebDAV 目录失败',
'music.webdav_truncated': '这个目录条目太多，只显示了前面一部分，请进入子目录查看。',
'music.up': '上一级',
'music.imported': '已添加到曲库',
'music.import_failed': '添加失败',
'music.upload_webdav_done': '已上传到 WebDAV',
'music.music_dir': '音乐子目录',
'music.public_music': '公开音乐库到博客',
'music.public_music_hint': '开启后博客前台可只读浏览与播放；上传、编辑与 WebDAV 仍只在本应用内。',
'music.public_music_on': '已公开',
'music.public_music_off': '未公开',
'music.public_music_unknown': '不可用',
'music.music_dir_hint': '音乐文件会存放在 WebDAV 目标下的这个目录中。',
'music.music_target': '音乐使用的 WebDAV 目标',
'music.music_target_auto': '第一个启用的 WebDAV 目标',
'music.sleep_timer': '睡眠定时',
'music.off': '关闭',
'music.sleep_minutes': '{value0} 分钟',
'music.sleep_remaining': '{value0} 后停止',
'music.sleep_after_current': '播完当前曲后停止',
'music.playback_rate': '播放速度',
'music.eq': '均衡器',
'music.eq_enable': '开启均衡器',
'music.eq_presets': '预设',
'music.eq_preset_flat': '平直',
'music.eq_preset_pop': '流行',
'music.eq_preset_rock': '摇滚',
'music.eq_preset_vocal': '人声',
'music.eq_preset_bass': '低音',
'music.eq_bass': '低音',
'music.eq_mids': '中音',
'music.eq_treble': '高音',
'music.normalize': '音量归一化',
'music.crossfade': '交叉淡化',
'music.immersive': '沉浸式播放',
'music.exit_immersive': '退出沉浸式',
'music.export_m3u': '导出为 M3U',
'music.select_track': '选择歌曲',
'music.source_filter': '按来源筛选',
'music.move_player': '移动播放器',
'music.move_player_hint': '用方向键移动播放器；激活后回到默认角落',
'music.rewind': '快退 10 秒',
'music.forward': '快进 10 秒',
'music.delete_remote_file': '删除 WebDAV 文件',
'music.delete_remote_confirm': '确定从 WebDAV 服务器删除《{value0}》吗？此操作不可撤销。',
'music.expand_mini_player': '展开浮动播放器',
}