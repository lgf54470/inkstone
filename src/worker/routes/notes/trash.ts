  ...
  notesRoutes.post('/trash/empty', async (c) => {
    ...
    if ((row?.count ?? 0) > 0) {
      const results = await c.env.DB.batch(
        trashPurgeStatements(c.env.DB, { userId, ftsEnabled, now: Date.now() }),
      )
...
function trashPurgeStatements(db: D1Database, params: {
  userId: string
  ftsEnabled: boolean
  now: number
}): D1PreparedStatement[] {
  const { userId, ftsEnabled, now } = params
  const trashed = `SELECT id FROM notes WHERE user_id = ?1 AND deleted_at IS NOT NULL`
  const statements = [
    db.prepare(`DELETE FROM note_tags WHERE note_id IN (${trashed})`).bind(userId),
    db.prepare(`DELETE FROM links WHERE source_note_id IN (${trashed})`).bind(userId),
    db.prepare(
      `UPDATE links SET target_note_id = ${LINK_TARGET_SUBQUERY}
        WHERE user_id = ?1 AND target_note_id IN (${trashed})`,
    ).bind(userId),
    db.prepare(`DELETE FROM note_versions WHERE note_id IN (${trashed})`).bind(userId),
    db.prepare(
    ...
    statements.push(
      ...
      db.prepare(
        `INSERT OR REPLACE INTO ai_index_queue (user_id, note_id, kind, created_at)
         SELECT ?1, id, 'delete', ?2 FROM notes WHERE user_id = ?1 AND deleted_at IS NOT NULL`,
      ).bind(userId, now),
      db.prepare(
        `INSERT INTO changes (user_id, entity, entity_id, op, at)
         SELECT ?1, 'note', id, 'delete', ?2 FROM notes WHERE user_id = ?1 AND deleted_at IS NOT NULL`,
      ).bind(userId, now),
      db.prepare(`SELECT seq FROM changes WHERE user_id = ?1 ORDER BY seq DESC LIMIT 1`).bind(userId),
      db.prepare(`DELETE FROM notes WHERE user_id = ?1 AND deleted_at IS NOT NULL`).bind(userId),
    )
    return statements
}