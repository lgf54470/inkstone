...
export function registerTagsCrudRoutes(tagsRoutes: Hono<AppBindings>): void {
  registerTagListRoute(tagsRoutes)
  registerTagCreateRoute(tagsRoutes)
  registerTagPatchRoute(tagsRoutes)
  registerTagDeleteRoute(tagsRoutes)
}

function registerTagListRoute(tagsRoutes: Hono<AppBindings>): void {
...
function registerTagCreateRoute(tagsRoutes: Hono<AppBindings>): void {
...
function registerTagPatchRoute(tagsRoutes: Hono<AppBindings>): void {
  tagsRoutes.patch('/:id', async (c) => {
    const userId = c.get('userId')
    const id = c.req.param('id')
    const body = await readJsonValidated(c, patchTagSchema, JSON_BODY_LIMITS.small)

    const tag = await loadTagRow(c.env.DB, userId, id)
    ...
    if (typeof body.name === 'string') {
      const next = validateTagName(body.name)
      if (next !== tag.name) {
        return patchTagRename(c, userId, tag, body, color, isPinned, next)
      }
    }
    return patchTagAppearance(c, userId, tag, color, isPinned)
  })
}

function registerTagDeleteRoute(tagsRoutes: Hono<AppBindings>): void {
  tagsRoutes.delete('/:id', async (c) => {
    ...
    try {
      const outcomes = await c.env.DB.batch(buildDeleteTagStatements(c.env.DB, userId, tag))
      if (!outcomes.at(-1)?.meta.changes) {
        throw ApiError.conflict('The tag changed elsewhere. Refresh and try again')
      }
    } catch (error) {
      await rollbackTagRewriteOrThrow(rewrite, 'Tag deletion could not be rolled back safely; refresh and try again')
      throw error
    }
    await broadcastCursor(c)
    scheduleFtsDrain(c)
...
async function loadTagRow(
  ...
  userId: string,
  id: string,
): Promise<TagRowLite | null> {
  return db.prepare(
    `SELECT id, name, color, is_pinned FROM tags WHERE id = ?1 AND user_id = ?2`,
  ).bind(id, userId).first<TagRowLite>()
}

async function loadTagByNameRow(
  db: D1Database,
  userId: string,
  name: string,
): Promise<TagRowLite | null> {
  return db.prepare(
    `SELECT id, name, color, is_pinned FROM tags
      WHERE user_id = ?1 AND name = ?2 COLLATE NOCASE
      ORDER BY created_at ASC, id ASC LIMIT 1`,
  ).bind(userId, name).first<TagRowLite>()
}
...
async function patchTagRename(
  c: Context<AppBindings>,
  userId: string,
  tag: TagRowLite,
  body: z.infer<typeof patchTagSchema>,
  color: string | null,
  isPinned: boolean,
  next: string,
): Promise<Response> {
  const existing = await c.env.DB.prepare(
    `SELECT id, name FROM tags
      ...
      ORDER BY created_at ASC, id ASC LIMIT 1`,
  ).bind(userId, tag.id, next).first<{ id: string; name: string }>()
  const destinationName = existing?.name ?? next
  const rewrite = await rewriteTagInNotes(c.env, c.get('database').ftsEnabled, userId, tag.id, tag.name, destinationName)
  const destination = await loadTagByNameRow(c.env.DB, userId, destinationName)
  const now = Date.now()
  try {
    if (destination && destination.id !== tag.id) {
      await mergeTagIntoDestination(c, userId, tag, destinationName, body, color, isPinned, now)
    } else {
      await renameTagInPlace(c, userId, tag, destinationName, body, color, isPinned, now)
    }
  } catch (error) {
    await rollbackTagRewriteOrThrow(rewrite, 'Tag rename could not be rolled back safely; refresh and try again')
    throw error
  }
  await broadcastCursor(c)
  scheduleFtsDrain(c)
...
async function renameTagInPlace(
  c: Context<AppBindings>,
  userId: string,
  tag: TagRowLite,
  ...
  const [updated] = await c.env.DB.batch([
    ...
    ).bind(tag.id, userId, tag.name, destinationName, explicitColor, color, explicitPinned, nextPinned),
    c.env.DB.prepare(
      `INSERT INTO changes (user_id, entity, entity_id, op, at)
       SELECT ?2, 'tag', ?1, 'upsert', ?4
        WHERE EXISTS (SELECT 1 FROM tags
          WHERE id = ?1 AND user_id = ?2 AND name = ?5)`,
    ).bind(tag.id, userId, tag.name, now, destinationName),
  ])
...
async function mergeTagIntoDestination(
  c: Context<AppBindings>,
  userId: string,
  tag: TagRowLite,
  destinationName: string,
  ...
  const statements = [
    c.env.DB.prepare(
      `INSERT INTO tags (id, user_id, name, color, is_pinned, is_manual, created_at)
       ...
       ON CONFLICT(user_id, name) DO UPDATE SET
         ...
         is_manual = 1`,
    ).bind(tag.id, userId, tag.name, targetId, destinationName, explicitColor, color, explicitPinned, nextPinned, now),
    c.env.DB.prepare(
    ...
    ).bind(tag.id, userId, tag.name, destinationName),
    c.env.DB.prepare(`DELETE FROM note_tags WHERE tag_id = ?1 AND ${sourceGuard}`)
      .bind(tag.id, userId, tag.name),
    c.env.DB.prepare(
      `INSERT INTO changes (user_id, entity, entity_id, op, at)
    ...
    ).bind(tag.id, userId, tag.name, now),
...
async function patchTagAppearance(
  c: Context<AppBindings>,
  userId: string,
  tag: TagRowLite,
  color: string | null,
  ...
  const pinChanged = (tag.is_pinned === 1) !== isPinned
  ...
  if (colorChanged || pinChanged) {
    ...
    ).bind(color, isPinned ? 1 : 0, tag.id, userId, tag.name)
    const change = c.env.DB.prepare(
      `INSERT INTO changes (user_id, entity, entity_id, op, at)
       SELECT ?1, 'tag', ?2, 'upsert', ?3
        WHERE EXISTS (SELECT 1 FROM tags WHERE id = ?2 AND user_id = ?1)`,
    ).bind(userId, tag.id, now)
...
function buildDeleteTagStatements(
  db: D1Database,
  userId: string,
  tag: TagRowLite,
): D1PreparedStatement[] {
  ...
  return [
    db.prepare(`DELETE FROM note_tags WHERE tag_id = ?1 AND ${guard}`)
      .bind(tag.id, userId, tag.name),
    db.prepare(
      `INSERT INTO changes (user_id, entity, entity_id, op, at)
       SELECT ?2, 'tag', ?1, 'delete', ?4 WHERE ${guard}`,
    ).bind(tag.id, userId, tag.name, now),
    db.prepare(`DELETE FROM tags WHERE id = ?1 AND user_id = ?2 AND name = ?3`)
      .bind(tag.id, userId, tag.name),
  ]
}

async function rollbackTagRewriteOrThrow(rewrite: TagRewriteResult, message: string): Promise<void> {
  try {
