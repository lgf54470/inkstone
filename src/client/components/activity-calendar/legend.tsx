...
export function HeatLegend(): JSX.Element {
  return (<div className='mt-1.5 flex items-center gap-1 px-1'>
    ...
    {[0, 1, 2, 3, 4].map((level) => (<span key={level} aria-hidden='true' className='size-2.25 rounded-[var(--r-2)]' style={{ backgroundColor: level === 0 ? 'var(--bg-inset)' : `color-mix(in oklab, var(--accent) ${HEAT_PERCENTS[level]}%, transparent)` }}/>))}
