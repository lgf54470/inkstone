...
export function DateRangePopover(props: DateRangePopoverProps) {
    ...
    if (!props.open)
        return null;
    return createPortal(<div ref={p.popoverRef} role="dialog" aria-label={t("notes.range_editor_title")} className="anim-pop fixed z-[var(--z-hover-card)] rounded-[var(--r-lg)] border border-[var(--border-default)] bg-[var(--bg-overlay)] p-2 shadow-[var(--shadow-pop)]" style={{ top: p.position.top, left: p.position.left, width: 248, transformOrigin: p.position.origin }}>
