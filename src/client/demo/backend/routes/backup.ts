...
function mergeBackupTargetInput(current: BackupTarget, input: BackupTargetPatchInput): BackupTargetInput {
  return {
...
async function createBackupTargetHandler(c: Context, state: DemoState): Promise<Response> {
...
async function patchBackupTargetHandler(c: Context, state: DemoState): Promise<Response> {
  ...
  const mergedInput = mergeBackupTargetInput(current, input)
  const invalid = demoBackupTargetError(mergedInput, input.type !== undefined && input.type !== current.type)
  if (invalid) return apiError(400, 'bad_request', invalid)
...
function deleteBackupTargetHandler(c: Context, state: DemoState): Response {
...
async function testBackupTargetHandler(c: Context): Promise<Response> {
  const input = await jsonBody(c.req.raw) as unknown as BackupTargetInput
  const invalid = demoBackupTargetError(input, true)
  return invalid
...
async function testBackupTargetByIdHandler(c: Context, state: DemoState): Promise<Response> {
  const current = state.backupTargets.get(c.req.param('id') ?? '')
  if (!current) return apiError(404, 'not_found', 'Backup target not found')
  const input = await jsonBody(c.req.raw) as unknown as BackupTargetPatchInput
...
async function runBackupHandler(c: Context, state: DemoState): Promise<Response> {
...
async function importSingleFile(value: File, state: DemoState, result: ImportResult): Promise<void> {
...
async function importHandler(c: Context, state: DemoState): Promise<Response> {
  ...
  for (const value of files) {
    try {
      await importSingleFile(value, state, result)
    } catch (error) {
...
export function registerBackupRoutes(app: Hono, state: DemoState): void {
  ...
  app.post('/api/backup/targets', (c) => createBackupTargetHandler(c, state))
  app.patch('/api/backup/targets/:id', (c) => patchBackupTargetHandler(c, state))
  app.delete('/api/backup/targets/:id', (c) => deleteBackupTargetHandler(c, state))
  app.post('/api/backup/test', (c) => testBackupTargetHandler(c))
  app.post('/api/backup/targets/:id/test', (c) => testBackupTargetByIdHandler(c, state))
  app.post('/api/backup/run', (c) => runBackupHandler(c, state))
  ...
  app.post('/api/import', (c) => importHandler(c, state))
}