...
function listNotesHandler(c: Context, state: DemoState): Response {
...
async function createNoteHandler(c: Context, state: DemoState): Promise<Response> {
...
function emptyTrashHandler(c: Context, state: DemoState): Response {
...
function listVersionsHandler(c: Context, state: DemoState): Response {
  const versions = state.versions.get(c.req.param('id') ?? '') ?? []
  return c.json({ versions: versions.map(({ content: _content, ...meta }) => meta) })
}

function restoreVersionHandler(c: Context, state: DemoState): Response {
...
function backlinksHandler(c: Context, state: DemoState): Response {
...
function restoreNoteHandler(c: Context, state: DemoState): Response {
...
async function duplicateNoteHandler(c: Context, state: DemoState): Promise<Response> {
...
function purgeNoteHandler(c: Context, state: DemoState): Response {
  const id = c.req.param('id') ?? ''
...
async function patchNoteHandler(c: Context, state: DemoState): Promise<Response> {
  const id = c.req.param('id') ?? ''
...
function deleteNoteHandler(c: Context, state: DemoState): Response {
  const note = state.notes.get(c.req.param('id') ?? '')
  if (!note) return apiError(404, 'not_found', 'Note not found')
...
export function registerNotesRoutes(app: Hono, state: DemoState): void {
  app.get('/api/notes', (c) => listNotesHandler(c, state))
  app.post('/api/notes', (c) => createNoteHandler(c, state))
  app.post('/api/notes/trash/empty', (c) => emptyTrashHandler(c, state))
  app.get('/api/notes/:id/versions', (c) => listVersionsHandler(c, state))
  ...
  app.post('/api/notes/:id/versions/:versionId/restore', (c) => restoreVersionHandler(c, state))
  app.get('/api/notes/:id/backlinks', (c) => backlinksHandler(c, state))
  app.post('/api/notes/:id/restore', (c) => restoreNoteHandler(c, state))
  app.post('/api/notes/:id/duplicate', (c) => duplicateNoteHandler(c, state))
  app.delete('/api/notes/:id/purge', (c) => purgeNoteHandler(c, state))
  app.get('/api/notes/:id', (c) => {
    const note = state.notes.get(c.req.param('id') ?? '')
    return note ? c.json(note) : apiError(404, 'not_found', 'Note not found')
  })
  app.patch('/api/notes/:id', (c) => patchNoteHandler(c, state))
  app.delete('/api/notes/:id', (c) => deleteNoteHandler(c, state))
}