...
function renameTagInState(state: DemoState, current: { id: string; name: string }, requestedName: string, color: unknown): number {
  ...
  state.tagColors.set(nextName, color === null || typeof color === 'string'
    ? color
...
async function patchTagHandler(c: Context, state: DemoState): Promise<Response> {
  ...
  if (typeof body.name === 'string' && body.name.trim() && body.name.trim() !== current.name) {
    ...
    const renamed = renameTagInState(state, current, requestedName, body.color)
...
export function registerTagsRoutes(app: Hono, state: DemoState): void {
  ...
  app.post('/api/tags', (c) => createTagHandler(c, state))
  app.patch('/api/tags/:id', (c) => patchTagHandler(c, state))
  app.delete('/api/tags/:id', (c) => deleteTagHandler(c, state))
}