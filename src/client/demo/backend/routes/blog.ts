...
function buildStats(posts: BlogPost[], comments: BlogComment[], categories: BlogCategory[], tags: BlogTag[]): BlogStats {
...
function buildRecentVisits(posts: BlogPost[]): BlogGlobalAnalytics['recentVisits'] {
  const published = posts.filter((post) => post.isPublished)
  const devices: Array<['desktop' | 'mobile' | 'tablet', string]> = [['desktop', 'Chrome'], ['mobile', 'Safari'], ['desktop', 'Edge'], ['mobile', 'Chrome'], ['desktop', 'Firefox']]
  const countries = ['中国', '美国', '日本', '德国', '新加坡']
  return published.slice(0, 5).map((post, index) => ({
    id: index + 1,
    postId: post.id,
    postTitle: post.title,
    slug: post.slug,
    visitedAt: SEED_NOW - index * 5 * 60 * 60 * 1000,
    country: countries[index] ?? '中国',
    region: null,
    city: null,
    referrer: index % 3 === 0 ? 'https://news.ycombinator.com/' : null,
    referrerHost: index % 3 === 0 ? 'news.ycombinator.com' : null,
    deviceType: devices[index]![0],
    os: index % 2 === 0 ? 'macOS' : 'Windows',
    browser: devices[index]![1],
    isBot: index === 4,
    botName: index === 4 ? 'Googlebot' : null,
  }))
}

function buildAnalytics(posts: BlogPost[], range: ShareTimelineRange): BlogGlobalAnalytics {
  ...
  return {
    ...
    recentVisits: buildRecentVisits(posts),
    filterStats: { bots: 24, selfReferrals: 7, owner: 3 },
  }
}

interface BlogDemoData {
  posts: BlogPost[]
  folders: BlogFolder[]
...
function createBlogDemoData(userId: string): BlogDemoData {
...
function registerBlogMetaRoutes(app: Hono, data: BlogDemoData): void {
  ...
  app.get('/api/blog/check-slug', (c) => c.json({ available: true }))
...
function registerBlogSettingsRoutes(app: Hono, data: BlogDemoData): void {
  ...
  app.patch('/api/blog/settings', async (c) => {
    data.settings = { ...data.settings, ...(await jsonBody(c.req.raw)) } as BlogSettings
...
function registerBlogPostRoutes(app: Hono, data: BlogDemoData): void {
...
function registerBlogTaxonomyRoutes(app: Hono, data: BlogDemoData): void {
...
function registerBlogCommentRoutes(app: Hono, data: BlogDemoData): void {
  ...
  app.post('/api/blog/comments/batch', async (c) => {
    ...
    if (action === 'delete') {
      const index = data.comments.findIndex((comment) => comment.id === ids[0])
      if (index >= 0) data.comments.splice(index, 1)
      return c.json({ ok: true as const, count: 1 })
    }
...
function registerBlogAnalyticsRoute(app: Hono, data: BlogDemoData): void {
  app.get('/api/blog/analytics', (c) => {
    ...
    return c.json({ analytics: buildAnalytics(data.posts, range) })
  })
}
...
export function registerBlogRoutes(app: Hono, state: DemoState): void {
  ...
  registerBlogPostRoutes(app, data)
  registerBlogTaxonomyRoutes(app, data)
