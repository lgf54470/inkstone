...
function listShareFoldersHandler(c: Context, state: DemoState): Response {
  return c.json({ folders: [...state.shareFolders.values()] })
}

async function createShareFolderHandler(c: Context, state: DemoState): Promise<Response> {
...
async function patchShareFolderHandler(c: Context, state: DemoState): Promise<Response> {
...
function deleteShareFolderHandler(c: Context, state: DemoState): Response {
  const id = c.req.param('id') ?? ''
...
function listShareTagsHandler(c: Context, state: DemoState): Response {
  return c.json({ tags: [...state.shareTags.values()] })
}

async function createShareTagHandler(c: Context, state: DemoState): Promise<Response> {
  const body = await jsonBody(c.req.raw)
...
async function patchShareTagHandler(c: Context, state: DemoState): Promise<Response> {
  const id = c.req.param('id') ?? ''
...
function deleteShareTagHandler(c: Context, state: DemoState): Response {
  const id = c.req.param('id') ?? ''
  const tag = state.shareTags.get(id)
  if (tag) {
    for (const share of state.shares.values()) {
...
async function batchToggleGroupHandler(c: Context, state: DemoState): Promise<Response> {
  const body = await jsonBody(c.req.raw)
...
async function batchFolderHandler(c: Context, state: DemoState): Promise<Response> {
  const body = await jsonBody(c.req.raw)
...
async function batchTagHandler(c: Context, state: DemoState): Promise<Response> {
  const body = await jsonBody(c.req.raw)
...
function shareNoteHandler(c: Context, state: DemoState): Response {
  const share = state.shares.get(c.req.param('noteId') ?? '')
  return c.json({ share: share ? absoluteShare(share.info, c.req.url) : null })
}

async function createShareHandler(c: Context, state: DemoState): Promise<Response> {
...
function deleteShareHandler(c: Context, state: DemoState): Response {
  state.shares.delete(c.req.param('noteId') ?? '')
  return c.json({ ok: true as const })
}

async function publicShareHandler(c: Context, state: DemoState): Promise<Response> {
...
export function registerShareAdminRoutes(app: Hono, state: DemoState): void {
  app.get('/api/share/folders', (c) => listShareFoldersHandler(c, state))
  app.post('/api/share/folders', (c) => createShareFolderHandler(c, state))
  app.patch('/api/share/folders/:id', (c) => patchShareFolderHandler(c, state))
  app.delete('/api/share/folders/:id', (c) => deleteShareFolderHandler(c, state))
  app.get('/api/share/tags', (c) => listShareTagsHandler(c, state))
  app.post('/api/share/tags', (c) => createShareTagHandler(c, state))
  app.patch('/api/share/tags/:id', (c) => patchShareTagHandler(c, state))
  app.delete('/api/share/tags/:id', (c) => deleteShareTagHandler(c, state))
  app.post('/api/share/batch-toggle-group', (c) => batchToggleGroupHandler(c, state))
  app.post('/api/share/batch-folder', (c) => batchFolderHandler(c, state))
  app.post('/api/share/batch-tag', (c) => batchTagHandler(c, state))
  app.get('/api/share/note-share/:noteId', (c) => shareNoteHandler(c, state))
  app.get('/api/share/:noteId', (c) => shareNoteHandler(c, state))
  app.post('/api/share/:noteId', (c) => createShareHandler(c, state))
  app.delete('/api/share/:noteId', (c) => deleteShareHandler(c, state))
  app.post('/api/public/:slug', (c) => publicShareHandler(c, state))
}