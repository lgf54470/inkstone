...
async function measureTypingBursts(bursts: number): Promise<BurstMeasurements> {
  ...
  vi.useRealTimers()
  return { burstKeys, burstBytes, bigValues, smallValueMs }
}

function reportTypingBenchmark(bursts: number, measurements: BurstMeasurements): void {
  const { burstKeys, burstBytes, bigValues, smallValueMs } = measurements
  console.log('')
...
describe('db shell benchmark', () => {
  ...
  it('measures per-burst flush traffic and serialization cost for a typing session', async () => {
    const bursts = 3
    const measurements = await measureTypingBursts(bursts)
    reportTypingBenchmark(bursts, measurements)

    // Regression guard for the two-level layout: a single-note edit must never
    // re-serialize the whole vault on the flush path.
    const wholeVaultSeen = measurements.bigValues.some((value) => value > 0)
    expect(wholeVaultSeen).toBe(false)
    expect(measurements.burstKeys.every((count) => count === 1)).toBe(true)
    expect(measurements.burstBytes.every((count) => count < fixture.oneSummaryBytes * 4)).toBe(true)
  }, 30_000)
})
