  ...
  return (
    <aside aria-label={t('templates.categories')} className='hidden w-54.5 shrink-0 flex-col overflow-y-auto border-r border-[var(--border-subtle)] p-2 md:flex'>
      ...
      {derived.tagList.length > 0 && (<>
        <div className="mt-3 mb-1 px-2 text-[length:var(--text-10\.5)] font-semibold tracking-[var(--tracking-label)] text-[var(--text-quaternary)]">{t('templates.tags')}</div>
        <div className='space-y-0.5'>
...
export function GalleryMain({ g }: { g: GalleryController }) {
  ...
  return (
    <main className='min-h-0 flex-1 overflow-y-auto p-3 md:p-4'>
      ...
      {filter.kind !== 'community' && derived.visible.some((item) => item.isPinned) && (<div className="mb-3 flex items-center gap-1.5 text-[length:var(--text-10\.5)] font-semibold tracking-[var(--tracking-label)] text-[var(--text-quaternary)]">
        <Pin size={11}/>{t('notes.pin')}
      </div>)}
      {filter.kind !== 'community' && derived.visible.length === 0 ? (<div className='flex h-full min-h-60 flex-col items-center justify-center gap-2 text-center'>
