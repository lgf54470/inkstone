...
export function TemplateGallery({ onClose }: {
    ...
    return createPortal(<div className="app-viewport-fixed fixed z-[var(--z-palette)] flex items-end justify-center md:items-center md:p-6">
        ...
        <div ref={state.panelRef} role="dialog" aria-modal="true" aria-label={t("templates.template_library")} tabIndex={-1} onKeyDown={g.handleKeyDown} className="anim-pop relative flex h-[min(88dvh,var(--app-viewport-height,100dvh))] w-full max-w-[940px] flex-col overflow-hidden rounded-t-[var(--r-2xl)] border border-b-0 border-[var(--border-default)] bg-[var(--bg-overlay)] pb-[env(safe-area-inset-bottom)] shadow-[var(--shadow-modal)] outline-none md:rounded-[var(--r-2xl)] md:border-b md:pb-0">
        ...
        <GalleryModals g={g}/>
        <Menu anchor={state.moreButtonRef} open={state.isMoreOpen} onClose={() => state.setIsMoreOpen(false)} items={g.moreItems} align="end" width={200} zIndex={Z_INDEX.menuHigh}/>
    </div>, document.body);
}