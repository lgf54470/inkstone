...
function syncLabel(
  ...
  if (saving) return t('shell.saving')
  if (dirty) return t('shell.unsaved_changes')
  return lastSavedAt ? t('shell.synced_value0', { value0: savedAgo }) : t('shell.synced')
}
...
export function SaveIndicator() {
  ...
  const label = syncLabel(offline, pending, saving, dirty, lastSavedAt, savedAgo)
  return offline ? <OfflineIndicator label={label} /> : <SyncGlyph label={label} saving={saving} dirty={dirty} synced={synced} />
}
