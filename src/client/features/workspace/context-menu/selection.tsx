...
function buildFormatItems(ctx: MenuCtx): MenuItem[] {
  const run = ctx.runStateCommand
  return [
    { id: 'bold', label: t('common.bold'), icon: <Bold size={13} />, combo: 'mod+b', onSelect: () => run(toggleBold) },
    { id: 'italic', label: t('common.italic'), icon: <Italic size={13} />, combo: 'mod+i', onSelect: () => run(toggleItalic) },
    { id: 'strikethrough', label: t('common.strikethrough'), icon: <Strikethrough size={13} />, combo: 'mod+shift+x', onSelect: () => run(toggleStrikethrough) },
    { id: 'underline', label: t('common.underline'), icon: <Underline size={13} />, combo: 'mod+u', onSelect: () => run(toggleUnderline) },
    { id: 'highlight', label: t('common.highlight'), icon: <Highlighter size={13} />, combo: 'mod+shift+h', onSelect: () => run(toggleHighlight) },
    ...
    { id: 'ruby', label: t('workspace.ruby_annotation'), icon: <Languages size={13} />, onSelect: () => run(insertRuby) },
    { id: 'code', label: t('common.inline_code'), icon: <FileCode size={13} />, combo: 'mod+e', onSelect: () => run(toggleInlineCode) },
    { id: 'math', label: t('workspace.inline_math'), icon: <Sigma size={13} />, onSelect: () => run(toggleInlineMath) },
  ]
}

function buildHeadingItems(ctx: MenuCtx): MenuItem[] {
  const run = ctx.runStateCommand
  return [1, 2, 3, 4, 5, 6].map((lvl) => ({
    id: `h${lvl}`,
    label: t('workspace.heading_value0', { value0: lvl }),
    combo: `mod+${lvl}`,
    onSelect: () => run(setHeading(lvl)),
  }))
}

function buildListItems(ctx: MenuCtx): MenuItem[] {
  const run = ctx.runStateCommand
  return [
    { id: 'bullet', label: t('common.unordered_list'), icon: <List size={13} />, combo: 'mod+shift+8', onSelect: () => run(toggleBulletList) },
    { id: 'ordered', label: t('common.ordered_list'), icon: <ListOrdered size={13} />, combo: 'mod+shift+7', onSelect: () => run(toggleOrderedList) },
    { id: 'task', label: t('common.task_list'), icon: <ListTodo size={13} />, combo: 'mod+shift+9', onSelect: () => run(toggleTaskList) },
    { id: 'quote', label: t('common.quote'), icon: <Quote size={13} />, combo: 'mod+shift+.', onSelect: () => run(toggleQuote) },
    { id: 'callout', label: t('workspace.callout'), icon: <Sparkles size={13} />, onSelect: () => run(insertCallout) },
  ]
}

function buildConvertItems(ctx: MenuCtx): MenuItem[] {
  const run = ctx.runStateCommand
  return [
