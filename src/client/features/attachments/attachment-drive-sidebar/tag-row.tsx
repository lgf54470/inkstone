...
interface TagMenuContext {
  node: TagTreeNode
  patchTag: DriveTagRowProps['patchTag']
  deleteTag: DriveTagRowProps['deleteTag']
}

function buildTagMenuItems(ctx: TagMenuContext): MenuItem[] {
  const { node } = ctx
  const castTag: Tag = {
  ...
  return [
    {
      ...
      icon: <Pin size={13} />,
      onSelect: () => {
        if (node.tag) void ctx.patchTag(node.tag.id, { isPinned: !node.tag.isPinned })
      },
    },
    {
      id: 'color',
    ...
    { id: 'delete', label: t('tags.delete'), icon: <Trash2 size={13} />, tone: 'danger', separatorBefore: true, onSelect: () => { if (node.tag) void ctx.deleteTag(node.tag.id) } },
  ]
}

export function DriveTagRow(props: DriveTagRowProps) {
  const { node, selectedTag, expandedTagPaths, onToggleExpand, onSelectTag, patchTag, deleteTag } = props
  ...
  const menuItems = buildTagMenuItems({ node, patchTag, deleteTag })
  ...
  return (
    <div>
      <TagRowBody
        ...
        onSelectTag={() => onSelectTag(node.fullPath)}
      ...
      />
      <TagRowMenus moreButtonRef={moreButtonRef} isMenuOpen={isMenuOpen} contextMenu={contextMenu} menuItems={menuItems} onCloseMenu={() => setIsMenuOpen(false)} />
    </div>
  )
}

interface TagRowBodyProps {
  node: TagTreeNode
  active: boolean
  expanded: boolean
  hasChildren: boolean
...
function TagRowBody(props: TagRowBodyProps) {
  const { node, active, expanded, hasChildren, moreButtonRef, onToggleExpand, onSelectTag, onMoreClick, onRowContextMenu } = props
  return (
    <div
...
function TagRowMenus(props: TagRowMenusProps) {
