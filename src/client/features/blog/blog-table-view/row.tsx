    ...
    return (
        ...
        >
            <td className="px-3 py-2.5 text-center">
                <input type="checkbox" checked={isSelected} onChange={onToggleSelect} className="size-3.5 rounded accent-[var(--accent)] cursor-pointer" />
            </td>
            <TableRowTitleCell post={post} />
            <td className="px-3 py-2.5"><ColoredBadge name={folder?.name} color={folder?.color} /></td>
            <td className="px-3 py-2.5"><ColoredBadge name={cat?.name} color={cat?.color} /></td>
            <TableRowTagsCell post={post} />
            <TableRowStatusCell post={post} />
            <td className="px-3 py-2.5 text-right font-medium text-[var(--text-secondary)]">{post.views}</td>
            <td className="px-3 py-2.5 text-right font-medium text-[var(--text-secondary)]">{post.commentsCount ?? 0}</td>
            <td className="px-3 py-2.5 text-[length:var(--text-11\.5)] text-[var(--text-quaternary)] whitespace-nowrap">{new Date(post.publishedAt).toLocaleDateString()}</td>
            <TableRowActionsCell row={row} post={post} onOpenEdit={onOpenEdit} />
        </tr>
    )
}
    ...
    return (
        <td className="px-3 py-2.5 text-right whitespace-nowrap">
            <div className="flex items-center justify-end gap-1">
                ...
                <IconButton size="sm" label={post.isPinned ? t('blog.unpin_post') : t('blog.pin_post')} onClick={() => void row.updatePost(post.id, { isPinned: !post.isPinned })} className={cn(post.isPinned ? 'text-[var(--accent)]' : 'text-[var(--text-quaternary)] hover:text-[var(--accent)]')}>
                ...
                <RowActionButton label={t('blog.copy_link')} onClick={() => void row.handleCopyLink(post.slug)}><Copy size={13} /></RowActionButton>
                <RowActionButton label={t('blog.post_settings')} onClick={() => onOpenEdit(post)}><Settings2 size={13} /></RowActionButton>
                <RowActionButton label={t('blog.sync_post')} onClick={() => void row.handleSync()}><RefreshCw size={13} /></RowActionButton>
                <RowActionButton label={t('common.delete')} onClick={() => void row.handleDelete()} danger><Trash2 size={13} /></RowActionButton>
            </div>
