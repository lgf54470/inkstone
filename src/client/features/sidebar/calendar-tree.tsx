...
import { CalendarDays, CheckSquare, FolderClosed, FolderOpen } from 'lucide-react'
...
function VirtualRowIcon({ active, isRoot, rootIcon, open }: {
    active: boolean
    isRoot: boolean
    rootIcon: React.ReactNode
    open: boolean
    ...
    return (
        <span className={cn('shrink-0', active ? 'text-[var(--accent)]' : 'text-[var(--text-tertiary)]')}>
            {isRoot
                ? rootIcon
                : <span aria-hidden="true" data-open={open || undefined} className="folder-motion-icon">
...
function VirtualRowLabel({ active, isRoot, rootLabel, name, tooltip, onOpen }: {
    active: boolean
    isRoot: boolean
    rootLabel: string
    name: string
