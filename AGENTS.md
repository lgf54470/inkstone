...
- 自动化 a11y 门禁：`axe-core` 作为 devDependency，用 `page.evaluate` 注入真实页面（不插 script 标签，不动 CSP），逐个场景断言违规为 0。按断言类别分两处：外壳的 `aside`、命令面板与设置对话框（层级/强调色实际绘制的表面）跟在对比度量之后跑在 `scripts/check-contrast.mjs`，两套主题各一遍——axe 读的是合成后的像素，与量测是同一种问题的两次提问，因此同主题同一次打开的实例才是同一个答案；演示浮层与侧栏留在 `scripts/e2e-visual.mjs`，只跑浅色（默认系统偏好的解析结果）。两个脚本共用 `scripts/e2e-harness.mjs` 的启动、登录（含按真实交互关掉 owner 更新提示）、主题切换与 axe 注入，各自只保留自己那类场景。三个 `incomplete` 类别按 id + 原因显式放行（重复渲染同一标记导致的 `aria-hidden-focus`/`duplicate-id-aria`、axe 对单字符与纯符号标签的自身保留、面板动画中途的 `partially obscured`），任何新的审查项仍会失败；测面板前必须等其 `opacity` 到位，否则会把入场动画读成对比度不足。
...
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 101 条断言，覆盖放映、导出、思维导图与演示浮层的 a11y；外壳与面板的 a11y 归 `scripts/check-contrast.mjs`）。该门禁的三处既定前提：登录后按真实交互关掉 owner 的更新提示（提示的遮罩会吞掉紧随其后的点击，表现为「探针正文未渲染」），这一步与两个门禁共用的其余启动动作都在 `scripts/e2e-harness.mjs`；主题反转场景先把账号主题设为「跟随系统」再翻转系统偏好（账号主题是上一轮运行留下的，`scripts/e2e.mjs` 结束时停在深色）；导图场景的节点选择等待库自己的 `selected` 类，笔记侧断言读已提交的栅栏体而不是编辑器文本（CodeMirror 只渲染视口内的行）。库在尚未显示的窗格里首次绘制产生的 NaN path 报错按消息放行，其余页面错误仍然致命。CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
