...
- 自动化 a11y 门禁：`axe-core` 作为 devDependency，由 `scripts/e2e-visual.mjs` 用 `page.evaluate` 注入真实页面（不插 script 标签，不动 CSP），逐个场景断言违规为 0：演示浮层与侧栏、命令面板、设置对话框。三个 `incomplete` 类别按 id + 原因显式放行（重复渲染同一标记导致的 `aria-hidden-focus`/`duplicate-id-aria`、axe 对单字符与纯符号标签的自身保留、面板动画中途的 `partially obscured`），任何新的审查项仍会失败。门禁只在浅色主题下运行（默认系统偏好的解析结果）；测面板前必须等其 `opacity` 到位，否则会把入场动画读成对比度不足。
- 对比度修正：两套主题的暗文字层级与各强调色均按 AA 校准——强调色需在自身 14% 软底上作为文字满足 4.5:1（因此比早期取值更深；暗色主题的 cinnabar 因此为 71.5%，66.5% 在 `--bg-editor` 的强调软底上只有 4.36），暗色主题的 `--text-tertiary`/`--text-quaternary` 需在 `--bg-sunken`…`--bg-raised` 上达标（早期 56%/43% 实测 4.02/2.33），活跃日历格的热度上限取 `HEAT_PERCENTS` 的 50%（70% 时 indigo 在暗色 3.58、graphite 在浅色 3.90）。文字层级落在强调底色上必须实测满足 AA：`npm run contrast:check`（`scripts/check-contrast.mjs`，CI 中跟在 e2e 之后对同一实例运行）量测两套主题里实际绘制的「层级 × 底色」组合（含半透明底色与其下层表面），再把每个带强调底色的组合按全部 7 个强调色重算一遍（强调可一键切换，账号只画其中一个），并单独校验每个强调色作为文字落在自身软底上的规则；非令牌（用户自定义标签色）只报告不判定。选中行的计数徽标因此取上一层文字层级（`features/sidebar/sidebar/count-badge.ts`），最暗层级在强调底色上只有 3.86:1，无法靠调令牌补救（要让它在 14% 强调底上达标，令牌会深于上一层、层级倒挂）。改动共享令牌后必须 `node scripts/check-token-drift.mjs --update-baseline`，漂移基线即改动记录。
...
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 111 条断言，覆盖放映、导出、思维导图与 a11y）。该门禁的三处既定前提：登录后按真实交互关掉 owner 的更新提示（提示的遮罩会吞掉紧随其后的点击，表现为「探针正文未渲染」）；主题反转场景先把账号主题设为「跟随系统」再翻转系统偏好（账号主题是上一轮运行留下的，`scripts/e2e.mjs` 结束时停在深色）；导图场景的节点选择等待库自己的 `selected` 类，笔记侧断言读已提交的栅栏体而不是编辑器文本（CodeMirror 只渲染视口内的行）。库在尚未显示的窗格里首次绘制产生的 NaN path 报错按消息放行，其余页面错误仍然致命。CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
