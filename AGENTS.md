...
- 常用命令：
  ...
  - 手动验收脚本（不进 CI）：`scripts/measure-preflight.mjs`——对长 deck 采样预量测的主线程占用与最坏帧（`WINDOW_MS`/`PARAS` 可调，打印数据与结论）；需要账号时用 `INKSTONE_VISUAL_USERNAME/PASSWORD`，需要浏览器时用 `INKSTONE_CHROME_PATH`
