...
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。导图调色板的两条规则（跟随应用设置、体自带 `theme` 优先）由 `src/client/lib/markdown/mindmap/vendor.test.ts` 驱动真实 mind-elixir 断言它写在元素上的内联色变量与分支色（jsdom 不布局，但这两样正是库自己写下的；registry 那组仍用桩，因为它们查的是接管而非配色），判定本身另在 `mindmap/theme.test.ts`。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 215 条断言，覆盖放映、导出、思维导图（含头部配色选择器写栅栏与重画）、演示浮层与全屏看板的 a11y（看板另含 dialog 命名、标题链、表格值编辑器命名，以及在浮层开着时改账号语言重绘），卡片的悬停行不得压住自己的标题（在笔记里、看板视图与画廊视图各量一遍：卡片在看板里画在笔记**外**，笔记的 prose 下边距不再遮蔽这类重叠；悬停只是改 `opacity`，所以量的是几何，headless 下 `@media (hover: hover)` 不成立也跑得动），以及逐表面的工具栏稳定性扫描；外壳与面板的 a11y 归 `scripts/check-contrast.mjs`）。该门禁的三处既定前提：登录后按真实交互关掉 owner 的更新提示（提示的遮罩会吞掉紧随其后的点击，表现为「探针正文未渲染」），这一步与两个门禁共用的其余启动动作都在 `scripts/e2e-harness.mjs`；主题反转场景先把账号主题设为「跟随系统」再翻转系统偏好（账号主题是上一轮运行留下的，`scripts/e2e.mjs` 结束时停在深色）；导图场景的节点选择等待库自己的 `selected` 类，笔记侧断言读已提交的栅栏体而不是编辑器文本（CodeMirror 只渲染视口内的行）。库在尚未显示的窗格里首次绘制产生的 NaN path 报错按消息放行，其余页面错误仍然致命。CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
