...
- 自动化 a11y 门禁：`axe-core` 作为 devDependency，用 `page.evaluate` 注入真实页面（不插 script 标签，不动 CSP），逐个场景断言违规为 0。按断言类别分两处：外壳的 `aside`、命令面板与设置对话框（层级/强调色实际绘制的表面）跟在对比度量之后跑在 `scripts/check-contrast.mjs`，两套主题各一遍——axe 读的是合成后的像素，与量测是同一种问题的两次提问，因此同主题同一次打开的实例才是同一个答案；演示浮层、侧栏与思维导图全屏留在 `scripts/e2e-visual.mjs`，只跑浅色（默认系统偏好的解析结果）。思维导图全屏是同一个覆盖层的两次提问：`e2e-visual.mjs` 在快捷键参考卡打开时测一次（关掉再测就等于把该面板新加的那个表面重新跳过），`check-contrast.mjs` 在依赖度量之后、两套主题各打开一次，先读含卡的一遍、再读同一实例上把卡收起的一遍——卡画在绘图区中央，它盖住的节点文字正是 axe 拒绝判定的那类「被另一元素遮挡」，只读一遍就只能把真实像素当成豁免，两遍才能既判卡、又判被它盖住的文字，且两边都不靠沉默放行。两个脚本共用 `scripts/e2e-harness.mjs` 的启动、登录（含按真实交互关掉 owner 更新提示）、主题切换、布局循环与 axe 注入，各自只保留自己那类场景。三个 `incomplete` 类别按 id + 原因显式放行（重复渲染同一标记导致的 `aria-hidden-focus`/`duplicate-id-aria`、axe 对单字符与纯符号标签的自身保留、面板动画中途的 `partially obscured`），任何新的审查项仍会失败；测面板前必须等其 `opacity` 到位，否则会把入场动画读成对比度不足。
