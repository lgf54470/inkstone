...
- 富媒体块：markdown 渲染在 `src/client/lib/markdown/`（`renderer` 出标记、`enhance` 挂交互、`mindmap` 为 `mind-elixir` 思维导图块）；`mindmap` 栅栏支持大纲（Markdown 列表）与 JSON 两种体，预览里由 registry 复用同一实例实现与笔记的双向同步，全屏复用 `Modal variant='fullscreen'`，分享/演示/导出走 `exportSvg` 静态快照；库与样式只在首次出现导图时按需加载（`scripts/check-vendor-isolation.mjs` 守卫）。内联实例刻意跨预览重渲染存活（保住相机、选中与撤销栈），所以主题切换必须显式下发给它：`applyTheme` 走 `changeTheme` + 重画连接线，因为库把主题的颜色变量写成元素内联样式、把分支调色板在画连接线时烧进节点，实例不会自己跟着 CSS 变——只有「刷新页面才变色」即回归，「切换即变色且画布元素未重建」由 `scripts/e2e-visual.mjs` 按内联变量与分支线颜色两重断言守住；静态快照与演示每次重画，不走这条。
...
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 147 条断言，覆盖放映、导出、思维导图与演示浮层的 a11y，以及逐表面的工具栏稳定性扫描；外壳与面板的 a11y 归 `scripts/check-contrast.mjs`）。该门禁的三处既定前提：登录后按真实交互关掉 owner 的更新提示（提示的遮罩会吞掉紧随其后的点击，表现为「探针正文未渲染」），这一步与两个门禁共用的其余启动动作都在 `scripts/e2e-harness.mjs`；主题反转场景先把账号主题设为「跟随系统」再翻转系统偏好（账号主题是上一轮运行留下的，`scripts/e2e.mjs` 结束时停在深色）；导图场景的节点选择等待库自己的 `selected` 类，笔记侧断言读已提交的栅栏体而不是编辑器文本（CodeMirror 只渲染视口内的行）。库在尚未显示的窗格里首次绘制产生的 NaN path 报错按消息放行，其余页面错误仍然致命。CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
