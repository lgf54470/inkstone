...
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 67 条断言，覆盖放映、导出与 a11y），CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
