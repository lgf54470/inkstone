...
- 对比度修正：浅色主题的暗文字层级（`--text-tertiary`/`--text-quaternary`）与各强调色均按 AA 校准——强调色需在自身 14% 软底上作为文字满足 4.5:1，因此比早期取值更深；活跃日历格的最高热度百分比同样按对比度封顶。改动共享令牌后必须 `node scripts/check-token-drift.mjs --update-baseline`，漂移基线即改动记录。
