...
- [ ] 改动最小化：无关格式化与无关重构已剥离；分批提交时每个快照自身都能通过门禁（含注释白名单），且没有夹带他人未完成的改动。
...
`.githooks/pre-commit` 对**工作区**跑静态门禁，而不是对即将提交的快照。于是一个文件同时属于两个提交（典型是 `scripts/check-comments.mjs`：它的注释白名单按文件分块，几个批次各带一块）时，不能为了凑出中间态把工作区退回上一版——那样门禁读到的是混合状态，必然报未登记的注释和已被删掉的条目。

做法是工作区始终停在最终状态，只把属于本批的 hunk 放进暂存区：`git diff <file> > /tmp/all.patch`，按 hunk 或按块切成 `part1/part2`，`git apply --cached part1.patch` 暂存本批，其余不动（切分后的补丁要保留末尾换行，否则 `git apply` 会报补丁损坏）。

暂存快照得单独验证，因为工作区已经被别的东西充满：`git archive HEAD | tar -x -C /tmp/snap`，把本批涉及的文件按暂存版本复制进去（库依赖用软链），在快照里跑对应门禁与测试。同理，每个批次都要带上自己那部分白名单条目：给不存在的注释留条目、或让新注释无条目，两个方向都会被门禁拒掉。

多人或多线程共用一个检出时，先确认工作区里没有别人的未完成改动（`git status`）再这么做；`scripts/sync-comments-allowlist.mjs` 会按当前工作区重建整张白名单，把别人的在途注释一并收进去。
...
- 常用命令：
  ...
  - 自定义门禁脚本：`style:check`、`size:check`、`comments:check`、`escape:check`、`empty-catch:check`、`hardcoded:check`、`tokens:check`、`i18n:check`、`module-state:check`、`deep-imports:check`、`surfaces:check`、`vendor:check`、`budget:check`；`blog-frontend` 复用 `size:check:blog`、`deep-imports:check:blog`；端到端 `test:e2e`（`scripts/e2e.mjs`、`scripts/e2e-visual.mjs`）
  - 手动验收脚本（不进 CI）：`scripts/measure-preflight.mjs`——对长 deck 采样预量测的主线程占用与最坏帧（`WINDOW_MS`/`PARAS` 可调，打印数据与结论）；`scripts/measure-kanban.mjs`——把 N 张卡的看板写进笔记并逐个切八视图，量首画 / 单次切视图 / 最长单次提交（`CARDS` 默认即栅栏上限 1000，另有 `NOTE_MS_MAX`/`VIEW_MS_MAX`/`WORST_TASK_MAX`；切视图的墙钟总账只报告不判定——八次刻意挂载的总和量的是脚本而非板）；两者需要账号时用 `INKSTONE_VISUAL_USERNAME/PASSWORD`，需要浏览器时用 `INKSTONE_CHROME_PATH`
...
- 可访问性基线：交互控件必须使用 `src/client/components`（primitives、form、overlay 等）基于原生语义元素；Overlay 组件实现焦点陷阱、ESC 关闭、焦点归还与 ARIA；状态变化用 `role="status"`/`aria-live`；颜色由 oklch 令牌保证对比度；`tokens.css` 在 `prefers-reduced-motion: reduce` 下将动效时长归零。
- 自动化 a11y 门禁：`axe-core` 作为 devDependency，用 `page.evaluate` 注入真实页面（不插 script 标签，不动 CSP），逐个场景断言违规为 0。按断言类别分两处：外壳的 `aside`、命令面板与设置对话框（层级/强调色实际绘制的表面）跟在对比度量之后跑在 `scripts/check-contrast.mjs`，两套主题各一遍——axe 读的是合成后的像素，与量测是同一种问题的两次提问，因此同主题同一次打开的实例才是同一个答案；演示浮层、侧栏、思维导图全屏与全屏看板留在 `scripts/e2e-visual.mjs`，只跑浅色（默认系统偏好的解析结果）；看板那一遍额外把放行收紧到自己的节点：唯一允许的 `incomplete` 是标签芯片里那个计数 span（芯片底色是自身文字色的 14% 半透明叠加，axe 解不出它背后的颜色，而这两个色对由 `check-contrast.mjs` 按令牌量测）。思维导图全屏在 `check-contrast.mjs` 里两套主题各打开一次、并在快捷键参考卡打开的状态下读（关掉再测就等于把该面板新加的那个表面重新跳过）——它只有一条 axe 判不了的项，且不是参考卡：库把节点文字画在自己定位（`position: relative`）的包装里，又给该文字 `pointer-events: none`，axe 的 grid 因此把包装排在它自己的孩子之上，报的是 `bgOverlap` 而不是比值。实测该项在「卡打开」「卡收起」「把文字临时改回可命中」三种状态下同样出现，故它是库自己的标记、不是本面板的层：同一实例读第二遍不可能干净，所以门禁不再假设遮挡者是卡，而是让**该表面自己**按名声明这一项（规则与分类器在 `scripts/lib/axe-review.mjs`：`id` + axe 的 `messageKey` + 目标形状 `me-tpc[data-nodeid] > .text` 三重同时命中才放行，`tests/axe-review.test.ts` 钉住三重各自承重），理由与命中条数一起打印在结果行里——该文字对是库自带主题（节点底是 `me-tpc` 的填充，不是本 app 的令牌），没有令牌可复测；形状变了、key 变了或换成任何别的审查项仍会失败，规则一旦不再命中也会以 0 的形式显示出来而不是沉默。两个脚本共用 `scripts/e2e-harness.mjs` 的启动、登录（含按真实交互关掉 owner 更新提示）、主题切换、布局循环与 axe 注入，各自只保留自己那类场景。三个 `incomplete` 类别按 id + 原因全局显式放行（重复渲染同一标记导致的 `aria-hidden-focus`/`duplicate-id-aria`、axe 对单字符与纯符号标签的自身保留、面板动画中途的 `partially obscured`），第四个（上段那条）声明在表面内、不全局生效，任何新的审查项仍会失败；测面板前必须等其 `opacity` 到位，否则会把入场动画读成对比度不足。
- 对比度修正：两套主题的暗文字层级与各强调色均按 AA 校准——强调色需在自身 14% 软底上作为文字满足 4.5:1（因此比早期取值更深；暗色主题的 cinnabar 因此为 71.5%，66.5% 在 `--bg-editor` 的强调软底上只有 4.36），暗色主题的 `--text-tertiary`/`--text-quaternary` 需在 `--bg-sunken`…`--bg-raised` 上达标（早期 56%/43% 实测 4.02/2.33），活跃日历格的热度上限取 `HEAT_PERCENTS` 的 50%（70% 时 indigo 在暗色 3.58、graphite 在浅色 3.90）。文字层级落在强调底色上必须实测满足 AA：`npm run contrast:check`（`scripts/check-contrast.mjs`，CI 中跟在 e2e 之后对同一实例运行）量测两套主题里实际绘制的「层级 × 底色」组合（含半透明底色与其下层表面），再把每个带强调底色的组合按全部 7 个强调色重算一遍（强调可一键切换，账号只画其中一个），并单独校验每个强调色作为文字落在自身软底上的规则；非令牌（用户自定义标签色）只报告不判定。外壳三个表面上的 axe 检查也由这门禁在同一主题、同一次打开的实例上跑完（见上一条），所以「选择行的计数徽标取上一层文字层级」这条规则由量测按令牌名、按全部强调色两重守住。选中行的计数徽标因此取上一层文字层级（`features/sidebar/sidebar/count-badge.ts`），最暗层级在强调底色上只有 3.86:1，无法靠调令牌补救（要让它在 14% 强调底上达标，令牌会深于上一层、层级倒挂）。改动共享令牌后必须 `node scripts/check-token-drift.mjs --update-baseline`，漂移基线即改动记录。
...
- 全屏归属：浏览器原生全屏只能由放映面板（`features/presentation/presentation-overlay.tsx`，元素稳定且自行跟踪 `fullscreenchange`）发起；任何第三方组件的自带全屏按钮都必须改走应用内叠加层——预览每次提交都会重挂载/重建组件的元素，元素一旦离开文档浏览器就会掉出全屏（mind-elixir 工具栏的 `#fullscreen` 就是这种情况，由 `mindmap/view.ts` 的 `disarmNativeFullscreen` 接管并在 `features/preview/mindmap-fullscreen.test.ts` 与视觉门禁中断言）。`tests/fullscreen-policy.test.ts` 扫描 `src/client` 保证没有第二处直接调用原生 API。`<video controls>`/`<audio controls>` 用浏览器自带控件，元素在重渲染中保持稳定，关闭模态即退出，无需叠加层。
- 工具栏展开：全屏/覆盖层的工具栏不得内联承载自己的展开面板——面板作为整行子元素（`flex: 1 1 100%` 这类）一打开就把头撑高、把刚点下的按钮推离指针（思维导图全屏的快捷键参考卡原本就是这样长出来的）。展开内容只有四种合格去处：内容区里的绝对定位浮层（导图 `.mindmap-shortcuts` 落在 `.mindmap-fullscreen-stage` 内居中，同类的还有音乐搜索历史的 `absolute top-full`、笔记列表底部的 `BulkBar`）、Portal（`Menu`/`Modal`/`Popover`）、独立一列或抽屉（大纲、反向链接、`Drawer`）、工具栏下方的独立横条（模板库与音乐的选中条）。审计结论：`fixed inset-0`/`app-viewport-fixed` 的 8 个全屏表面里只有思维导图覆盖层属于这一类，已改为绘图区浮层，回归由 `features/preview/mindmap-fullscreen.test.ts` 与 `scripts/e2e-visual.mjs` 的「头部高度差为 0」断言守住；其余表面都把展开内容放在上述四种位置之一，头部里没有会撑高工具栏的面板（图谱头部只随断点换行）。这条结论不再靠人复核：`scripts/e2e-visual.mjs` 末尾的 `assertFullscreenToolbars` 按人打开的路径逐个打开每个全屏表面（图谱、模板库、设置、命令面板、放映、灯箱、手机断点的抽屉外壳），逐个用真实指针点击工具栏里的每个切换控件，断言工具栏高度与控件位移为 0，并先断言表面带内容打开（打开即 500 的图谱曾让高度断言在空壳上通过）；思维导图覆盖层由它自己的场景断言同一件事加参考卡落点；模态外壳的全屏变体由四个使用者共用——导图覆盖层、幻灯片编辑器、手机断点的分享中心与全屏看板：前两者与分享中心各有自己的具名断言（分享中心在同一个元素上断言全屏变体确实铺满手机视口），看板的顶栏由扫描名单直接开合它的三个切换控件。每个表面声明自己的断点、开启它的那条路径（按可访问名称在两个语言下找同一个控件，快捷键路径先把键盘给到该控件）与「内容已到」的最小证据，带图内容按浏览器实际解码的图片数计；退出时除了断言 Escape 关掉它，还断言焦点回到打开它的那个控件——掉到 body 或掉进已经关掉的表面都算失败。这份名单不会自己长出来：`scripts/check-surface-coverage.mjs`（`npm run surfaces:check`）用 AST 找出 `src/client` 里所有靠 `app-viewport-fixed`/`fixed inset-0` 成为全屏表面的根，要求每一个要么出现在扫描名单里，要么由门禁里属于它的那些具名断言代表（一个表面根被多个浮层共用时，每个使用者各出一条），并把理由写在该脚本内；两个方向都失败——新表面（或共用表面根的新使用者）没人读会挂，名单里留下已经不存在的表面也会挂。
- 富媒体块：markdown 渲染在 `src/client/lib/markdown/`（`renderer` 出标记、`enhance` 挂交互、`mindmap` 为 `mind-elixir` 思维导图块）；`mindmap` 栅栏支持大纲（Markdown 列表）与 JSON 两种体，预览里由 registry 复用同一实例实现与笔记的双向同步；JSON 体可写 `"theme"`（`"light"`/`"dark"`/`"auto"` 或主题对象，见 `mindmap/theme.ts`）自己定配色，大纲体则在同一条栅栏的 info 串写 ` ```mindmap theme=dark `（渲染成 `data-mindmap-theme`，registry 再交给同一个 reader，所以两处判定不会漂移；两处都写以体自带字段为准，注解只是兜底，读不出来的注解与读不出来的字段一样走错误态）；它优先于应用明暗设置——写死的按写死的画，`auto` 或没写才跟随应用，所以切主题只重画跟随应用的那批、不动钉死的；块头部有一个配色选择器（`mindmap-theme-menu.tsx` + 渲染器出的 `[data-mindmap-theme-pick]` 按钮，React 侧用 `Menu`，因为 prose 白名单不放表单控件），选择经 `writeFence` 一次改写栅栏：JSON 体写进体自带字段并清掉注解，大纲体写 info 串的注解，选 `auto` 则两处都清——一处编辑、一次撤销即回到原样，重渲染后按钮的文案必须由 registry 重新写回（渲染器只出默认文案，实例活着就不会再走创建路径）；解析不了的取值走错误态（显示源码 + 提示），不静默换配色，自定义主题对象缺字段时按 `type` 叠在库的基础主题上（库画连接线时读 `theme.palette` 且不回退），全屏复用 `Modal variant='fullscreen'`，分享/演示/导出走 `exportSvg` 静态快照；库与样式只在首次出现导图时按需加载（`scripts/check-vendor-isolation.mjs` 守卫）。内联实例刻意跨预览重渲染存活（保住相机、选中与撤销栈），所以主题切换必须显式下发给它：`applyTheme` 走 `changeTheme` + 重画连接线，因为库把主题的颜色变量写成元素内联样式、把分支调色板在画连接线时烧进节点，实例不会自己跟着 CSS 变——只有「刷新页面才变色」即回归，「切换即变色且画布元素未重建」由 `scripts/e2e-visual.mjs` 按内联变量与分支线颜色两重断言守住；静态快照与演示每次重画，不走这条。

分享模块的三份决策记录（访客会话视角、`?ref=` 渠道标记、公开集合落地页）见根目录 `ADR-0003-visitor-session-view.md`、`ADR-0004-channel-tagging.md`、`ADR-0005-public-collection-pages.md`：会话与渠道的采集口径、集合成员的派生规则（以及"撤销集合 ≠ 撤销分享"这类容易被反向理解的行为）均以那里为准，改动这些行为前先读它们的"后果/局限"段。

设计令牌位置：`src/client/styles/tokens.css` 与 `blog-frontend/src/styles/tokens.css`（共享契约，`npm run tokens:check` + `scripts/check-token-drift.baseline.json` 守卫漂移）；z-index 见 `src/client/lib/z-index.ts`；`npm run hardcoded:check` 禁止内联视觉字面量。不走 CSS 的渲染物（库把颜色写进内联样式/SVG/canvas 的那些）必须显式跟随主题：渲染物清单、统一下发/重画约定与「刻意不跟随」的例外清单见 `ADR-0002-renderer-theme-following.md`——读一次 `getComputedStyle` 就把颜色冻在创建时刻即回归（图谱面板 canvas 是当前已知缺口，修复方向写在该 ADR 内）。
- 块内排版边界：富媒体块在笔记里画在 prose 之内，在它自己的全屏里画在 prose 之外，而 prose 是按元素声称的（`h1`–`h6`、`p`、`ul`/`ol`/`li`、`img`、`input[type=checkbox]`…）且未进层（unlayered），所以写在元素自身上的 utility 在笔记里一律输给 prose：卡片标题曾因此在笔记里是 18.24px/上边距 26.4px、在覆盖层里是 14px/无边距，凡是靠继承取字号的元素（徽标、`svg`）也随父级分别继承 16px/26.4px/-0.08px 与 13px/20.15px/normal。看板的做法是在 canvas 上声明自己的基准字型（`.kanban-canvas`：`--font-ui`/`--text-13`/`--ui-line`/`letter-spacing: normal`，即 body 的基准；`--ui-line` 是 body 与看板共用的唯一来源），并在 `.ink-prose [data-kanban-canvas]` 内把 prose 对块自身标记的元素主张交还（标题的 margin/字号/字重/行高/字距/`position`、段落的 margin 与 `text-wrap`、列表的 margin/padding/`list-style`、`img` 的装饰），交还值即块标记写作时依据的 preflight；块的快照（`.kanban-snapshot`，画在 canvas 旁边而非里面）保留它本来就要的 prose 列表样式。于是看板里的标题不自己带字号字重，而是放在包裹元素上（笔记对包裹没有规则），`kanban-card.tsx`、`kanban-gallery-view.tsx`、`kanban-empty-board.tsx`、`kanban-calendar-view.tsx`、`kanban-comments.tsx`、`kanban-item-detail-description.tsx`、`kanban-item-detail-fields.tsx` 都按这条写；卡片的标题与描述间距改由包裹的 `gap` 提供（描述自己的 `mt-1` 正是 prose 交还后会消失的那一侧）；画廊封面是 canvas 里唯一的 `img`，prose 的 `img { height: auto }` 属于 utility 拿不回来的那一类，故其盒子由 `.kanban-cover` 声明。prose 的 task-list 复选框皮肤只属于任务列表（`.contains-task-list`），否则笔记里每个富媒体块的复选框都会被画成 1.05em 的圆角 inset 方框（`prose/lists.css`）。这条不靠读代码维持：`scripts/e2e-visual.mjs` 的 `assertKanbanCanvasBase` 断言 canvas 的基准字型等于 body 的（而非笔记的），`readKanbanViewsInline` 在打开覆盖层之前把八个视图逐个在笔记里画一遍读其排版，`assertKanbanViews` 在覆盖层里再读一遍，`assertKanbanViewParity` 逐元素比对两处（只比决定「怎么排字」的属性；盒尺寸与解析后的网格轨道随面大小而变，不参与比对）；按压视图标签前会先确认指针真的落在该标签上——笔记的滚动区伸到 app 固定底栏之下，标签可能被测到那块底栏后面，而一次落在音乐控件上的真实按压会去操作它。
- 演示模式导出：页面由 `src/client/features/presentation/deck-print.tsx` 统一构建（同一分页结果、同一已渲染标记），两条出口分别是打印管线（PDF）与 `deck-image.ts`（SVG + canvas 栅格化为 PNG，`client-zip` 打包）；导出面板离屏布局而非 `display: none`（canvas 需要真实尺寸），且带 `inert`（prose 标记里的按钮不得进入 Tab 序）。
- 测试策略：Vitest 双工程（`vitest.config.ts`）：`jsdom` 覆盖客户端与多数测试，`node` 覆盖 worker/D1/demo 测试；同目录 `*.test.ts` + `tests/` 跨模块回归（基座 `tests/d1-harness.ts`，node:sqlite）。以行为为准，修 bug 先写复现；Markdown 渲染用带显式断言的快照；canvas 类测试共用 `src/client/lib/markdown/enhance.test-helpers.ts` 的 2D 上下文桩；富媒体块的「缓存重挂载」回归在 `src/client/features/presentation/slide-remount.test.ts`（图表、Mermaid、可运行示例各一组）。导图调色板的两条规则（跟随应用设置、体自带 `theme` 优先）由 `src/client/lib/markdown/mindmap/vendor.test.ts` 驱动真实 mind-elixir 断言它写在元素上的内联色变量与分支色（jsdom 不布局，但这两样正是库自己写下的；registry 那组仍用桩，因为它们查的是接管而非配色），判定本身另在 `mindmap/theme.test.ts`。`blog-frontend` 有 markdown 基线与 SSR 冒烟。端到端 `npm run test:e2e`（对 :7712 的全新本地实例）与 `scripts/e2e-visual.mjs`（Puppeteer，当前 380 条断言，覆盖放映、导出、思维导图（含头部配色选择器写栅栏与重画）、演示浮层与全屏看板的 a11y（看板另含 dialog 命名、标题链、表格值编辑器命名，以及在浮层开着时改账号语言重绘），卡片的悬停行不得压住自己的标题（在笔记里、看板视图与画廊视图各量一遍：卡片在看板里画在笔记**外**，笔记的 prose 下边距不再遮蔽这类重叠；悬停只是改 `opacity`，所以量的是几何，headless 下 `@media (hover: hover)` 不成立也跑得动），以及逐表面的工具栏稳定性扫描；外壳与面板的 a11y 归 `scripts/check-contrast.mjs`）。该门禁的四处既定前提：登录后按真实交互关掉 owner 的更新提示（提示的遮罩会吞掉紧随其后的点击，表现为「探针正文未渲染」），这一步与两个门禁共用的其余启动动作都在 `scripts/e2e-harness.mjs`；主题反转场景先把账号主题设为「跟随系统」再翻转系统偏好（账号主题是上一轮运行留下的，`scripts/e2e.mjs` 结束时停在深色）；导图场景的节点选择等待库自己的 `selected` 类，笔记侧断言读已提交的栅栏体而不是编辑器文本（CodeMirror 只渲染视口内的行）。看板在笔记里不止一块板：视图扫描写自己的三张卡（`gate-a`…），控件命名场景写自己的那张（`gate-controls-1`，带子任务与状态列）并按自己声明的卡片定位——两个场景各按「我那块板在不在」决定写不写、也只读自己那块，问「屏幕上有板吗」会让后跑的场景读到先跑的那块，子任务展开器与状态单元格随之都不在（合并时实测 13 条断言因此变红）。库在尚未显示的窗格里首次绘制产生的 NaN path 报错按消息放行，其余页面错误仍然致命。CI 以 `INKSTONE_EPHEMERAL_DEV=1 npm run dev:kv` 运行。
...
- CI 门禁：`.github/workflows/ci.yml` 主任务依次执行 `typecheck`、`test:unit`、`ci-bench-report`、`i18n:check`、`comments:check`、`escape:check`、`empty-catch:check`、`module-state:check`、`deep-imports:check`、`surfaces:check`、`style:check`、`hardcoded:check`、`tokens:check`、`size:check`、`size:check:blog`、`deep-imports:check:blog`、`build`、`check-bundle-budget`、`vendor:check`，再对本地实例跑 `scripts/e2e.mjs`、`scripts/e2e-visual.mjs` 与 `scripts/check-contrast.mjs`；`blog-frontend` 任务独立跑 `typecheck`、`lint`、`test`、`build`、smoke。`.githooks/pre-commit` 在本地镜像大部分静态门禁，并对暂存 TS 文件跑增量 `tsc -b` 与 `vitest related`。
