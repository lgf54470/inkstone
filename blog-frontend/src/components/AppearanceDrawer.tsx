...
export default function AppearanceDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end animate-in fade-in duration-200">
      ...
      {/* Drawer Panel */}
      <aside
        className="relative w-full max-w-sm bg-[var(--bg-surface)] text-[var(--text-primary)] border-l border-[var(--border-default)] shadow-2xl flex flex-col h-full z-10 transition-transform duration-300 ease-out"
        role="dialog"
        aria-modal="true"
        aria-label="Appearance Settings"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)] bg-[var(--bg-raised)]">
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-1.5 py-2.5 px-2 rounded-lg border text-xs font-medium transition-all ${
                      active
              ...
              {ACCENT_OPTIONS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative flex flex-col items-center gap-1.5 p-2 rounded-lg border text-xs transition-all ${
                      active
        ...
        {/* Footer */}
        <div className="p-4 border-t border-[var(--border-subtle)] bg-[var(--bg-raised)] flex items-center justify-between">
          <button
            type="button"
            onClick={resetToDefault}
            className="flex items-center gap-1.5 text-xs text-[var(--text-tertiary)] hover:text-[var(--text-primary)] px-2 py-1.5 rounded transition-colors"
          >
          ...
          <button
            type="button"
            onClick={() => setIsOpen(false)}
            className="px-4 py-1.5 rounded-lg bg-[var(--accent)] text-white font-medium text-xs hover:opacity-90 transition-opacity shadow-xs"
          >
            {isZh ? '完成' : 'Done'}
          </button>
        </div>
