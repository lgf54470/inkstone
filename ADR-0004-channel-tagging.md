...
Status: Accepted（已实现，2026-09-21）

Date: 2026-09-21

实现落点（本节随实现补写）：`src/shared/share-channel.ts`（字符集、两个保留名、`storedChannelValue`、`withChannelParam`）、migration 41（`share_visits.channel`）、`routes/share/public.ts` 的写入路径、`routes/share/channel-split.ts`（share 专属的分解语句）、`share-helpers.localizeChannelName` 与看板/单篇分析两处卡片、二维码面板与批量条的标记输入、账号设置开关。**未采信的一处**：标记只在**新建的访问行**上写入（30 分钟去重窗口内的后续访问不产生行，因此它的标记不会被记录）——这是去重口径的既有推论，已在 `public.ts` 注脚与下方「后果」里写明，不做补偿。
...
- 边界与拒绝：`src/shared/share-channel.test.ts` 钉住 32/33 字符边界、大写/空格/点/百分号编码/非拉丁/控制字符/前导短横线，以及两个保留名不可被访客占用（`__unmarked__` 与 `__unrecognized__` 都不匹配 token 正则）。
- 写入路径：`tests/share-routes.test.ts` 的 `share channel marker collection (ADR-0004)` 一组，含"被拒标记仍写行、只把标记存成空串""超长标记 400 而不是截断""关闭后不落任何标记且访问照常计数""标记不进入 referrer/referrer_host"以及日志接口的下发。
- 分解：同一组钉住三行（未标记 3 / 标记 2 / 未识别 1）与百分比分母（= 视图数），并在单篇分析接口上重复同一断言，保证两个界面不会给出不同统计。
- 客户端：`share-qr-marker.test.ts`（码、展示链接、复制、外链四处同源；无效值不改链接且报 `aria-invalid`），`share-dashboard-channels.test.ts`（两个保留名文案互不相同、提示只在不带标记时出现、标记只以文本节点渲染、无访问时不画这一节），`share-batch-links.test.ts`（批量标记走同一个 `withChannelParam`）。
- CSV：`share-visit-logs-csv.test.ts` 的表头与首行断言已含末尾 `Channel` 列。
- **未做**：标记的采集量与索引代价没有真实 D1 读数（列可空、只在已收窄的区间内读，故未建索引）——留待有量时再测，不预估。
...
1. **默认开启**。采的是所有者自己写进链接的东西，不是访客的隐私数据；关闭后写入端完全不落标记（不是落了不展示）。开关落在账号设置（`ShareSettings.collectChannel`），因为它改的是服务端写入行为。
2. **不做按链接的多标记维度**。一条分享一个 slug 的模型不变，多渠道 = 多条分享；引入"标记维度"会把分享模型从一对多改成多对多，超出本 ADR 范围。
3. **无效标记单独计数**（`share.channel_unrecognized`），确认视觉上可接受：它只在看板的渠道分解里占一行，且与"未标记"分列——把两者并成一行就等于把"标记失效"藏起来。
4. **二维码只写进 URL，不印可见文字**。码面留白；标记在面板里以输入框回显、在链接文本里可核对，印在码面会与"安静区"和扫码可靠性打架。

另外两处实现细节在实现时定下：**存储值用 `''` 表示"带了标记但被拒"**（而不是另存原文或加一个布尔列），因为有效 token 至少 1 字符，空串不会与任何合法标记冲突，且原文从不落库；**拒绝的判定用同一个 `normalizeChannelToken`**，生成端与服务端各调一次，不复制正则。
