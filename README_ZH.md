...
左侧列表是**页面列表**而不是分节列表：每一页都有一条自己的缩略图，点选即跳到该页。会分页的 `---` 幻灯片会把自己的每一页都列出来，因此通篇不用 `---` 的笔记同样有完整的侧栏。放映开始后会在空闲时段于后台量测整个 deck，每次空闲只量一张，下一张的等待时间按上一张的实际耗时拉长，因此填一份很长的 deck 也不会占住主线程。列表一开场就列出全部页面，包括还没放映到的幻灯片。放映时控件会自动淡出，移动鼠标或按下按键后重新出现。

「导出幻灯片为 PDF」走浏览器自带的打印管线：导出的每一页都对应放映里的一页，用的是同一份量测结果与同一份已渲染标记，因此在打印对话框里「另存为 PDF」得到的讲义与放映逐页一致。
