...
左侧列表是**页面列表**而不是分节列表：每一页都有一条自己的缩略图，点选即跳到该页。会分页的 `---` 幻灯片会把自己的每一页都列出来，因此通篇不用 `---` 的笔记同样有完整的侧栏。放映开始后会在空闲时段于后台量测整个 deck，所以列表一开场就列出全部页面，包括还没放映到的幻灯片。放映时控件会自动淡出，移动鼠标或按下按键后重新出现。
