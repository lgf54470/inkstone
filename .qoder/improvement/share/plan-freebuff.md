...
| SH-90 | `scripts/e2e-visual.mjs` 的「slides editor: space and a drag pan the view」 | 同一份代码、同一实例连跑三次得到两种结果（1 绿 2 红，失败时 `scrollLeft` 停在 0），当时归因于机器被另一条线程的浏览器门禁占满——**这个归因是错的**，与负载无关（§56） | **§56 已定位并修好**：不是等待窗口的问题，而是全局 Space 热键（音乐播放/暂停）在 window capture 阶段 `preventDefault()`＋`stopPropagation()` 把键抢走（键的目标是 `body`，所以它那道「在控件里」的守卫拦不住），有音轨时幻灯片编辑器的 Space 变成切歌。修法：`claimsSpace` 补 `[role="button"]` 与「在屏的 Space 归属表面」两支；断言改成先等应用自己的 armed 状态再按。旧读数里 1 绿 2 红的差别是**那一刻队列里有没有音轨** |
...
| SH-94 | `src/client/lib/i18n.ts` 的 `initI18n()`（`void ensureLocaleLoaded(other)`）——**§58 已关闭** | 预加载另一语言是个**没有 catch 的游离 Promise**（AGENTS 铁律 2）：机器被占满时那个动态 import 会 reject，变成 vitest 的 `Errors 1` 并把退出码变成非 0——第 11 项第一次提交就是这样被 pre-commit 钩子拒掉（同一命令重跑即绿，实测 1176/1176 + 无 error） | 按铁律 2 给这个 best-effort 预加载加 catch ＋ 注释（必要时 `console.warn` 最低级别日志），使其失败不影响调用方；CI 里也可考虑 `dangerouslyIgnoreUnhandledErrors` 以外的显式处理 |
| SH-93 | `src/client/components/hub-folder-row.tsx`（内含 `role='button'` + `tabIndex` 的 div、`FolderMoreButton`）、`components/hub-tag-item.tsx`（`TagMoreButton`、`TagExpandAffordance`）、`components/overlay/submenu.tsx`（`RowButton` 之外的裸按钮） | SH-49 只清了 `features/share`，而它用的共用组件里还有同类写法——包括 AGENTS 铁律 10 明禁的「`div` ＋ `onClick` ＋ `role='button'` 假冒控件」（`FolderRow`/`HubTagRow` 的行本体是 `div role='button'`）。第 48 项补了一条现场读数：账号一旦有标签，**分享中心自己的 axe 断言**就会以 `button-name`/`nested-interactive` 变红——CI 里那两条断言之所以是绿的，只是因为夹具账号既无标签也无访问（同源的还有 SH-102/103） | 把 SH-49 的守卫正则从 `features/share` 扩到 `src/client/components` 与其它 feature（分文件开口子、每个口子写理由），再逐处收；`div role='button'` 的行本体应换成真 `button` 或用项目组件，并补键盘路径回归。**§51 部分闭合**：`hub-tag-item.tsx`/`hub-folder-row.tsx` 两条共用行的行本体已改真 `button`、展开箭头与更多菜单按钮补上可访问名（新增 `src/client/components/hub-row-a11y.test.ts` 6 例守着形状）。**§53 闭合余下两半**：守卫改为 AST、扫整个 `src/client`（三条规则，见 §53），共用层 12 个文件逐条写明理由，`overlay/submenu.tsx` 与 `menu.tsx` 各自手写的菜单行合并为 `overlay/menu-row.tsx`；其余 feature 的 146 文件 / 397 处裸按钮只被要求“有可访问名、不是假控件”，**不要求**改走组件体系（理由与读数见 §53 局限①）。**§55 闭合最后一层**：这条规则刚补上的名字原本只有它自己读得到，现在附件、博客、看板、幻灯片四个表面各自在浏览器里被 axe ＋ 可访问性树读到（`assertNamedControlSurfaces`，58 条断言），读者第一次跑就量到 6 类真缺陷（无名的选择框/全选、3.19:1 的 amber 文字、跳级的 `h4`、重复的 `main`、无名 `<select>` ×2、藏在 `display:none` 里的图层按钮），均先红后绿；9 发变异各自被指名断言杀死（含一发「门禁仍绿 ⇒ 把无验证的属性删掉」） |
...
| SH-99 | `scripts/e2e-visual.mjs` 的 `LABELS.share*` 与 `scripts/check-contrast.mjs` 的 `SHARE_LABELS`——**§58 已关闭** | 两个门禁各自保存了同一套「分享中心」文案对（打开同一个表面、按同一个控件），一旦一侧改了文案另一侧会以「找不到控件」失败而告终 | 把那两个打开器（含文案对与定位规则）提到 `scripts/e2e-harness.mjs`，两个门禁共用一份；顺便把 `openShareHub` 的手机路径也一并收进去 |
| SH-100 | `scripts/check-contrast.mjs` 的 `openMusicHubList`——**§58 已关闭**（根因不是「本机缺种子」，而是夹具本身是假数据） | 在 `scripts/e2e.mjs` 刚跑过（177/0）的全新临时实例上，该门禁停在「找不到 `列表视图/List view`」而崩掉；本轮把分享中心表面排在它前面、后面都试过，两次同样崩在这里，而分享中心表面在跳过这三个音乐表面后在两套主题下都能跑完并全绿。未确认是本机音乐种子缺失还是门禁与种子之间的隐式契约 | 先弄清该门禁对音乐库前置数据的真实依赖（种子里是否真有可播放曲目），再决定是让门禁自行备好前置数据、还是把它标为需要预置实例 |
...
### 55 — SH-93 的另一半：四个表面在浏览器里被 axe 与可访问性树读到（2026-09-22）

- 前情（§53 自己写下的局限③）：守卫已扩到整个 `src/client`，但**它刚刚补上的那 25 个名字只有它自己读得到**——而这条静态规则在它自己的头注释里就写着读不到两类东西：wrapper 注入的名字，以及把 `{icon}` 这种表达式算成名字（第 53 项实测，浏览器把其中 8 个图标按钮判为无名）。源码能说「名字写下了」，只有浏览器能说「屏幕阅读器听到了什么」。本节给请求点名的四个表面（附件、博客、看板、幻灯片）各自找了一个真读者。
- 做法（`scripts/e2e-visual.mjs` 新增 `assertNamedControlSurfaces`，含 4 个表面函数 ＋ 一个夹具）：按人打开的路径开每个表面，把它**驱动到控件出现的那个状态**（搜索框输入、选中一行、切到表格/列表视图、折叠一列、打开子任务），然后读两遍：
  - **axe**：违规项必须落在注册表里（`SURFACE_A11Y_ALLOWANCES`，两侧失败——多出来的违规红，注册项没被读到也红），未复核项必须落在 `isReviewedIncomplete` 或该表面自带、**带实测的**允许里；
  - **浏览器自己的可访问性树**：CDP `DOM.getDocument` ＋ `Accessibility.getFullAXTree`，按真实 DOM 包含关系裁剪出该表面的节点，读它们的名字。
  - 名字表 `NAMED_CONTROLS` 中英成对，**它就是断言本身**（locale 文件里的串与浏览器报的串必须同串，`{value0}` 这类插值用 `RegExp`）。两个裸 `<select>` 另用逐控件读者 `checkControlName`（`Accessibility.getPartialAXTree`，只取那一个节点）：面级搜索会被 option 自己的名字满足——这条是变异 M 实测出来的，不是预防性设计。
  - 夹具只造**最少**的数据：一个文件（`gate-controls.txt`，带 `gate-tag`）、一篇已发布文章（slug 固定）、一条友链；看板与幻灯片栅栏按其他场景的写法追加进当前笔记，所以这一段可以单独跑。
- **读者第一次跑就找到的真缺陷**（先红后绿，逐条附读数）：
  1. 附件列表视图里**行首的选择框与表头的全选按钮没有任何名字**：axe `button-name`（摘掉行选择框的名字 → 7 处；摘掉表头那个 → 1 处 ＋ 同一条读数里带出的 `empty-table-header`）。补 `attachments.select_file`（`{value0}` 是文件名）与 `attachments.select_all` ＋ `aria-pressed`。
     - 顺带**删掉自己先加上的一个东西**：表头 `th` 的 `aria-label={attachments.select_all}` 单独摘掉后门禁**仍绿**（56/0），说明它不满足任何规则，只是把同一个名字在两个层级上重复宣告——按「不新增无验证的属性」删掉，只留按钮自己的名字。
  2. 附件三处 ＋ 博客三处的 amber 文字（`text-amber-600 dark:text-amber-400`）在**白底上只有 3.19:1**（axe 报 `#e17100` on `#ffffff`，中文字号，即「清理未引用」那颗按钮），换成令牌 `--warning`（该令牌已按「自身 -soft 底上作为文字 ≥4.5」校准，见对比度门禁）。
  3. 看板：卡片标题原本是 `h4`（在板子的 `h2` 下一跳两级）→ axe `heading-order`；`KanbanMain` 是 `<main>` 而外壳已有页面 `main` → axe `landmark-no-duplicate-main`（板视图 ＋ 表视图各一处）。改成 `h3` 与 `div`。
  4. 看板表视图的状态格是一个裸 `<select>`，无名字 → axe `select-name`（3 行各一处）；补 `aria-label`（列名，缺省回落 `preview.kanban_prop_status`）。
  5. 博客友链视图的分类筛选同样是裸 `<select>` → `select-name`；补 `aria-label`（既有键 `blog.link_category`）。
  6. 幻灯片图层行：两个排序按钮藏在 `hidden group-hover:flex` 里——**隐藏子树不在可访问性树里**，键盘到不了、屏幕阅读器读不到，axe 也整棵跳过（这是全客户端唯一一处这样藏悬停控件的地方）。改成 `opacity-0 … group-hover:opacity-100 focus-within:opacity-100`（与 `features/tags/tag-row.tsx`、侧栏自己的行同一串）。
  7. 静态守卫的那条读法（§53 的规则 2）也按这次量到的情况收紧：**只有渲染文本的表达式才算名字**（`rendersElement()` 递归看一眼表达式里有没有 JSX），并把 `{t('…')}`/`{name}` 这类不带元素的表达式仍算文本，`components/tags/tag-row.tsx` 作为「名字由 wrapper 组件的子节点渲染」的例外逐条写入 `COMPONENT_NAMED_BUTTONS`（含理由，且两侧失败）。
- **变异（9 发，各自被指名断言杀死；都是同一实例上的真实读数）**：
  1. 摘掉行选择框的名字 → `attachment drive: axe finds nothing on it that this gate has not registered`（`button-name` ×7）＋ 选中行那颗驱动断言跟着红（驱动用的就是同一个名字）；
  2. 摘掉表头全选的名字 → 同一条（`button-name` ×1 ＋ `empty-table-header`）；
  3. 摘掉表头 `th` 的 `aria-label` → **门禁仍绿**（56/0）→ 该属性不满足任何规则，已删除（见上）；
  4. amber 文字改回去 → 同一条（`color-contrast` 3.19:1，两处：网格与列表）；
  5. 卡片标题改回 `h4` → 同一条（`heading-order`）；
  6. `KanbanMain` 改回 `<main>` → 同一条（`landmark-no-duplicate-main` ×2）——第一次跑时我的替换脚本改错了闭合标签，门禁以「应用没渲染」崩掉（只跑了 2 条），重做后才是这条读数；
  7. 图层行改回 `hidden group-hover:flex` → `slides editor (layer list): every control it names is a name the browser reports`（两个名字从 115 个名字的树里消失）；
  8. 看板状态格摘掉 `aria-label` → 新增的逐控件断言（`name=""`）＋ axe `select-name` ×3；
  9. 友链分类筛选摘掉 `aria-label` → 同理（`name=""` ＋ `select-name` ×1）——这一发还先暴露了我自己断言的弱点：面级搜索会被同面里的 option 名字满足，因此才有逐控件读者；
 10. 摘掉文件夹图标选择器的 `aria-label`（它只有静态规则读）→ `tests/client-raw-controls.test.ts` 的「names every raw button it can read」直接报出 `pickers.tsx:16`。
- 验证读数：**浏览器门禁 299 通过 / 0 失败**（新增 58 条断言）；同一门禁在修前是 **289 通过 / 7 失败**，7 条红全部落在本节的表面上（其中 2 条属 §56 的 pan）。静态门禁 12 项全绿（`comments` 796 文件 / 5884 条；`size` 未动基线——把两处超长函数拆开而不是去改基线；`hardcoded` 因摘掉 7 处写死的 amber 而缩小，按门禁提示 `--update-baseline`）。定向单测（`tests/client-raw-controls.test.ts` ＋ `features/attachments`、`features/blog`、`features/music`、`lib/markdown/kanban`、`lib/markdown/slides`）→ **119 文件 / 871 用例全绿**。**全量串行回归**（`npx vitest run --config vitest.config.ts --no-file-parallelism --testTimeout=30000`）→ **349 文件 / 2817 通过 ＋ 1 skipped / 0 失败**；对 §54 的 349/2814 为 **＋3 用例 / ＋0 文件**，可核对：`music-hotkeys.test.ts` 5 → 7（§56 的两例）、`tests/client-raw-controls.test.ts` 6 → 7（新增的「例外条目自身要诚实」一例），合计正好 ＋3。
- 局限（如实登记）：① 覆盖的是请求点名的四个表面；其余位置的可访问名仍由各自的读者守着（分享中心是它自己的 axe 断言，侧栏的标签/文件夹行是 `check-contrast.mjs` 的 `aside` 那遍），本节不假装读全仓；② 名字断言按「这个名字在该表面里出现过」的形状检查，除两处裸 `select` 外都不绑定到具体控件——同面里若有另一个同名控件，可能替它满足断言（M9 就是这么被发现的）；③ 两个登记不修的允许项：`kanban:nested-interactive`（卡片根同时是点击目标又装着自己的控件 → SH-107）、`kanban:color-contrast`（看板自带的标签配色 → SH-108）；④ 另外三处 amber→令牌的替换（`blog-comments-view.tsx`、`link-card-row.tsx`、`link-checker-modal.tsx`）所在表面**没有**浏览器读者，理由是同一个缺陷类、同一处白底；⑤ 演示模式的后端没跑这条门禁；⑥ 四个表面都只在桌面视口读（手机断点下附件网格、看板表格的落版未读）。

### 56 — 「空格＋拖拽平移」不是负载敏感：全局 Space 热键把键抢走了（2026-09-22）

- 前情：SH-90 把 `slides editor: space and a drag pan the view` 登记为「负载敏感」（§54 也在局限里把它当噪声记了一笔：同一实例连跑三次 1 绿 2 红）。本节先把它当缺陷查，查完的结论是：**它从来不是负载问题**。
- 现场读数（探针用完即删，同一实例、真实路径：从笔记里的卡片打开幻灯片全屏）：
  - `page.keyboard.down(' ')` 之后 5s 内舞台 `cursor` 始终是 `auto`、没有任何 armed 痕迹；
  - 在 window/document 上挂三个监听后按真实 Space：**window capture 阶段 `defaultPrevented` 已经是 `true`**，事件只到 capture，`document` 与 window bubble 都没收到；
  - 同一时刻读 `<audio>`：**按键之前 DOM 里没有音频元素，按键之后多了一个带 stream 的音频元素**——即 `music-play-pause` 热键执行了 `togglePlay()`；
  - 手工在 window 上 `dispatchEvent(new KeyboardEvent('keydown', { key: ' ' }))` 时，cursor 立刻变 `grab`，随后拖拽平移 80px 生效——hook 一直在，是**键没到它手里**。
- 根因：`src/client/features/music/music-hotkeys.ts` 的 `space` 热键（播放/暂停）注册在热键注册表，而注册表用 `window.addEventListener('keydown', onKeyDown, { capture: true })` 绑定——它在 **window capture** 阶段调 `preventDefault()` ＋ `stopPropagation()`，而 `useSpaceHeld`（幻灯片平移）挂在 window **bubble**、presenter 与模板库挂在各自子树，都比它晚；`claimsSpace` 只挡了「目标在 button/a/input… 里」这一类，**没有**把「这个键是某个表面的手势」算进去。一个没聚焦的覆盖层里，键的目标就是 `body`，于是「在控件里」这道守卫根本拦不住它。
- 修法（同一根因，一次提交）：
  - 控件清单补 `[role="button"]`——标签 pill 是 `span` ＋ `role=button` ＋ `tabIndex`，Space 必须激活它，而它不在 `button/a/input…` 里；
  - 新增 `SPACE_OWNING_SURFACES`（`.bento-slides-fullscreen`、`.bento-slides-presenter`、`[data-surface="templates"]`）并按**在屏**判断（`document.querySelector`）而不是目标祖先——三者都自己用 Space（平移 / 下一张 / 选中卡片），且都不靠焦点接收它。presenter 根因此补上模块自己的类名 `bento-slides-presenter`（与 `bento-slides-fullscreen` 同一惯例）。
- 这是**产品缺陷**，不是门禁问题：有音轨时，幻灯片编辑器里 Space 不再平移而去切歌（编辑器帮助卡里就写着 Space 是平移）、演示模式里 Space 不再翻页、模板库里 Space 不再选中卡片。旧读数里「1 绿 2 红」的差别不是机器负载，而是**那一刻队列里有没有音轨**。
- 验证：
  - 探针复读（同一实例）：真实 Space 现在走完 capture → document → window bubble 三段，window bubble 上 `prevented: true`、cursor `grab`，`<audio>` 按键前后都是 `null`（没有起播，也不再挂上播放器）；拖拽平移 80px 生效，中键平移 60px 亦生效。
  - 单元：`src/client/features/music/music-hotkeys.test.ts` 5 → 7 例（`role=button` 的 pill 不被认领；三个表面在屏时不被认领；同时断言 plain ground 上仍认领）。两发变异各自被指名断言杀死（摘 `[role="button"]` → pill 那条红；摘 `.bento-slides-fullscreen` → 表面那条红）。
  - 门禁：`slides editor: space arms the pan before the press reaches the stage` 与 `slides editor: space and a drag pan the view` 修前**两条全红**（289/7 那次），修后全绿（299/0）。断言本身也改了：先等应用**自己**的 armed 状态（`cursor: grab`，即 `isPanReady` 画出来的那个状态）再按，而不是把两个协议调用背靠背发出去。
- 局限（如实登记）：① 断言等的是「应用说它准备好了」，所以它守的是这个竞态；键被第三方抢走这类问题在等不到 armed 时会由那条新断言红出来（本次就是这样），而不是静默放过；② presenter 与模板库的 Space 由同一条 `when` 放出，但这两处的**手势**没有浏览器级断言（只在探针里读过编辑器那一条），一并登记为 SH-109；③ 修法是一张**选择器清单**：将来新增一个「自己用 Space」的表面必须记得加进去，而没有任何门禁会因此变红——这是 SH-109 的第二半；④ 本节只改 `when`，没有动热键引擎的优先级（更晚注册的热键仍抢不过更早注册的），那是另一个题目。

### 57 — 本轮请求的收口与队列现状（2026-09-22）

> 本节只是状态汇总，不重复读数：每个读数都在它自己的小节里。工作树停在 `89cc6ca6`，`git status` 为空——本轮没有未提交的改动。

- 本轮请求与交付逐条对照（前 4 项一次提出，后 3 项是执行中追加的）：

| 请求 | 交付 |
| --- | --- |
| 选择规则收敛为一个共享谓词层，worker 的 SQL 与客户端过滤器共用 | §45（`9b8b7644`） |
| 分享中心每个分类各自成视图（各自的数据 hook），不再在列表壳里特判 | §46（`4cae175b`） |
| 基于 `?ref=collection` 的逐集合读数 | §47（`49ec94d6`） |
| 公开集合落地页的浏览器级场景（口令解锁 → 目录打开笔记 → 带渠道记下访问） | §48（`483418c8`） |
| SH-93 的另一半：守卫扩到 `src/client` 与共用层，`overlay/submenu.tsx` 的裸控件 | §53（`ac020acf` + `0138ec5d`） |
| 只有静态规则读可访问名的四个表面，在浏览器门禁里得到真读者 | §55（源码修复 `4090372b` ＋ 门禁 `89cc6ca6`） |
| 「空格＋拖拽平移」那条断言：查清是确定性还是负载敏感 | §56（产品修复 `ef19045e` ＋ 断言 `89cc6ca6`） |

- **已批准的 39 项：全部交付**（表中 37 ✅ ＋ 2 条按决策的部分交付：SH-64 的 PNG/PDF 评估后不做、重开条件在 §35；SH-68 的集合级资产会话按记录不做、重开条件在 §43）。队列表**没有待回填的 hash**（表中已无 ⏳）。
- **新发现 23 条：4 条已闭合，19 条仍登记不修**（按 AGENTS「不顺手修无关问题」）。已闭合：SH-90（§56 更正归因并关闭）、SH-93（§51 ＋ §53 ＋ §55 三层逐步闭合）、SH-102 与 SH-103（§51）。仍登记的分三类：
  - **需要产品决策的**：SH-92（分享子菜单的行高 40 vs 触控目标 44 的取舍）、SH-91（两个「分享」入口同名不同表面）、SH-107（看板卡片「一个点击目标 ＋ 自己的控件」的卡片面重设计）。
  - **校准 / 契约类**：SH-108（看板自带标签色板按 AA 重标并进对比度门禁）、SH-100（音乐夹具契约）、SH-99（两个门禁各存一套分享文案对）、SH-94（`initI18n()` 的游离 Promise 无 catch）、SH-105（`Tooltip` 与触发器无程序化关联）。
  - **同类缺陷的另一半**：SH-86/SH-87（另两处 CSV 写入点、blob URL 撤销时机）、SH-88（演示后端跳过 slug 校验）、SH-89/SH-98/SH-104（blog 侧的同形问题，属 blog 自己的范围）、SH-101（访问去重是先查后写、无指纹时不生效）、SH-95/SH-96（自定位浮层的窄屏裁切）、SH-97（思维导图的负载敏感断言，与 SH-90 同形但未定位）、SH-109（Space 归属是一张没有门禁的清单）。
- 结论（一句话）：**本轮请求的七件事全部交付并经门禁验证；队列里没有未交付的已批准项**——剩下的 19 条都是登记项，其中 **3 条需要产品决策**（SH-91/92/107）、**5 条是校准或契约**（SH-94/99/100/105/108）、**11 条是“同类问题的另一半”**（SH-86/87/88/89/95/96/97/98/101/104/109，其中 blog 侧一批按范围红线不在本仓修改）。
- 口径（如实登记）：① 本节不引入新读数；文中引用的三处终局读数（浏览器门禁 **299 通过 / 0 失败**、全量串行 **349 文件 / 2817 通过 ＋ 1 skipped / 0 失败**、定向 **119 文件 / 871 用例**）分别来自 §55/§56 与它们的提交前验证，跑的都是已提交的那棵树；② “全部交付”指**请求范围内的代码与门禁**，不等于全部登记项已闭合（上表右侧一列写的就是还没做的）；③ 本轮三次提交都是本地的，未 push（分支 `share-improvement-freebuff`，按台账约定等指令）。

### 58 — 五项校准与契约登记一起关闭（SH-94/99/100/105/108，2026-09-22）

> 用户本轮按「一批」请求关闭 §57 列出的五项校准 / 契约登记。两个提交：`01e279b4`（五项修复与门禁）＋ `04da1db6`（音乐夹具）。工作树 `git status` 为空，未 push。

- 逐项交付与证据：

| 登记 | 改了什么 | 证据 |
| --- | --- | --- |
| SH-94 游离的 i18n 预加载 | `initI18n()` 的后台预加载加 catch ＋ 最低级别日志（`console.warn('[inkstone] preloading … failed')`），失败只损失一次「提前量」，`setLocale()` 仍按需加载 | `i18n.test.ts` 新增 1 例：把待预加载的 locale 块 mock 成 reject，断言 `initI18n()` 仍 resolve 且日志里出现该语言。**变异**：删掉 catch → 该例红，并同时报 `Unhandled Rejection` 与 `Errors 1`——正是这条登记的原始症状 |
| SH-105 提示与触发器无关联 | `Tooltip` 用 `useId` 给面板一个 id，**仅在面板在屏时**把它加到触发控件的 `aria-describedby` 上（名字仍由 `aria-label` 提供）；克隆逻辑抽成模块级 `describedTrigger` | `overlay/tooltip.test.ts` 4 例（面板未起时无引用、在屏时引用指向该面板、触发器已有描述时追加、多元素触发器不猜引用）。**变异两发**各被杀掉：去掉「在屏才挂」的守卫 → 第一例红；整段克隆去掉 → 两条红。**门禁自己抓到过第一版**：先写成「总是挂引用」，axe 在九个表面上读到 3–56 处 `aria-valid-attr-value`（指向未挂载的面板），据此改成按在屏状态挂载 |
| SH-108 看板自带标签色板 | 12 色按 AA 重标为 `oklch`（浅色列 ≈47–50% L、暗色列 ≈71–74.5% L），底色改为由自身前景 `color-mix(… 14%, transparent)` 派生。旧的两半确实漂移过：暗色主题的底色写的是**浅色主题的十六进制**（如 pink 底色 rgba(193,76,138) 而前景 #e255a1） | 对比度门禁新增矩阵读取器（见下），逐色逐表面报名字。**变异**：把 blue 改回 `#337ea9` → 门禁红 24 条，读数 4.47:1（白底）与 3.89:1（自身软底），与 §55 的 axe 读数同源；恢复后 0 失败 |
| SH-99 两套分享文案对 | 文案对、打开器（含手机路径）与 opener 标记一起提到 `e2e-harness.mjs` 的 `openShareCenter`；`e2e-visual.mjs` 删掉本地 `openShareHub` 与四个重复文案对，`check-contrast.mjs` 删掉本地 `SHARE_LABELS` / `SHARE_HUB_DIALOG` / 打开器 | 两个门禁都跑过同一份：对比度门禁（分享中心表面，两套主题）与 e2e-visual 的分享场景（桌面 ＋ 手机断点、Escape 焦点归还）全绿 |
| SH-100 音乐夹具契约 | 根因**不是**「本机缺种子」：旧夹具在 `e2e-visual.mjs` 里把 35 字节文本当 `.mp3` 上传，注释还写着「这一段永远不解码」；而对比度门禁的音乐表面**确实会起播**。实测：demuxer 打不开 → 引擎报「网络太慢」→ 自动跳过 → 连击断路器提示盖住传输行 → 打开沉浸式播放的那一次点击落在提示上 → 门禁以「等不到对话框」收场。修法：夹具归 `e2e-harness.mjs` 一份，且以「浏览器真能打开它服务的那条流」（`loadedmetadata`）为准；同标题下打不开的行删掉重传，只有标题下没有可播放行时才上传；附带修好我第一版 WAV 头的 `fmt` 块长度（写成 36 而非 16，demuxer 直接拒绝）并把时长设为 60 秒（短曲目会中途自动跳到库里别的曲目） | 空库实例上首次调用 `uploaded: 2`、`found` 两条均可播放；之后的调用 `uploaded: 0`（幂等）；本机留下的 32 字节假曲目被自愈删除，库里回到两条 480,044 字节的真 WAV。对比度门禁的音乐三表面（列表 / 网格 / 沉浸式）在这台实例上两套主题全绿 |

- 门禁侧改动（SH-108 与 SH-100 的落点，都在 `scripts/check-contrast.mjs`）：
  - 三套「自身软底上作为文字」的色板（强调色、状态色、看板标签色）合并为一个在页内读取器 `TOKEN_MATRIX(kind)`；强调与状态色的读数与合并前逐字一致（7/7、3/3 条，0 失败），只是计数行多了一个「N 比值」。
  - 看板标签色按 **两套主题 × 两种背景变体**读，每变体 12 色 × 6 个不透明表面 × 两条规则（自身软底上、素底上）＝ 144 个比值，失败逐条报「哪个色、哪条规则、哪个表面」。
  - `e2e-visual.mjs` 撤掉 `kanban:color-contrast` 的允许项（该门的允许表两个方向都会红，所以撤掉即等于让读者重新判它）。
  - 音乐表面以「夹具真的可播放」为前提；打开音乐库与沉浸式播放前新增 `waitForHittable`（按键前确认该控件是真正会被点到的那个，等不到则打印挡路者），把「提示盖住了控件」与「表面没打开」分成两件事。

- 终局读数（本机全新 `dev:kv` 实例 7733，同一台机器上有别的线程在跑浏览器门禁）：
  - 静态门禁 12 项全绿：`comments`（5,966 条）＋ `style` / `size` / `escape` / `empty-catch` / `module-state` / `deep-imports` / `surfaces` / `hardcoded` / `tokens` / `i18n` / `vendor`，另 `tsc -b` 干净。
  - 对比度门禁：**全绿，0 失败**（九个表面 × 两套主题 ＋ 四套色板矩阵）。
  - 行为门禁 `e2e-visual.mjs`：**299 通过 / 0 失败**。
  - 全量串行：**350 文件 / 2822 通过 ＋ 1 skipped / 0 失败**（比 §57 的 349/2817 多出的正是本轮新增的 `overlay/tooltip.test.ts` 4 例与 i18n 的 1 例）。
  - 提交钩子（`.githooks/pre-commit`）另跑了一次增量门禁：静态 12 项 ＋ 暂存相关 188 文件 / 1268 用例全绿。

- 局限（如实登记）：
  - ① 三套色板共用同一个矩阵读取器，但矩阵读的是「声明的值 ＋ 自己合成」，与「屏幕上真的画出来的像素」仍是两次提问：像素那一次由 `e2e-visual.mjs` 的 axe 读者守着，而它只跑浅色、且只看到该账号画出的那几种标签色（深色与其余色只有矩阵这一遍）。
  - ② 本节只关 SH-108 的**配色**；同一张卡片上「一个点击目标又装着自己的控件」（SH-107）是另一件事，仍未修。
  - ③ `waitForHittable` 等不到可点时不重试按键，只打印挡路者后继续——后续断言会红，失败信息里因此有「谁挡的」，但它本身不是断言。
  - ④ 音乐夹具现在每条 480 KB（60 秒静音 WAV），且会在共享实例上删除「同标题且打不开」的行：若将来有第三个门禁也用这两个标题，会被它清掉（目前只有这两个门禁用）。
  - ⑤ SH-100 的**原始症状**（空库时找不到「列表视图」控件）在本机已没有可复现的实例（曲目已存在），本节读到的是「夹具自愈 ＋ 三个音乐表面全绿」；空库那条路径由首次调用 `uploaded: 2` 的读数单独作证，不是同一次运行里读到的。
  - ⑥ 两项一个提交（而不是五项五个提交）：五项共用 `scripts/check-contrast.mjs`、`e2e-harness.mjs` 与同一份注释白名单，而白名单是按文件分块的——按 §57 记录的 hunk 切分会让中间快照过不了 `comments:check`，代价大于收益；用户本轮也是按「一批」请求的。
