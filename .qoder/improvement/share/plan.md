...
- 安全边界（放行清单的改动必须反向断言，否则「修好了」等于「关掉了守门人」）：补丁后的实例上 `/@fs/<主检出>/vite.config.ts` 仍 403、`/@fs/home/kubuntu/.ssh/id_ed25519` 仍 403，而 `/@fs/<主检出>/node_modules/katex/dist/katex.min.css` 200、两支字体 200。即放行的只有依赖目录，不含主检出的源码与 `.dev.vars`。另澄清一条看着可疑但**与本改动无关**的读数：`/@fs/<主检出>/.dev.vars` 回 307，`Location` 是把 `@` 重新编码成 `%40` 的同一 URL，跟随后落到 SPA 外壳（`text/html`、与 `index.html` 同字节数量级），不是密钥文件；且这条 307 在未打补丁的实例上完全相同（先测过），属 Vite 既有行为。
...
- 验证：`tsc -b --force` 绿；13 项静态门禁全绿（`comments:check` 首跑拦下新注释，按流程 `sync-comments-allowlist.mjs` 登记后复绿：564 文件/3760 条）；全量回归 待回填。
