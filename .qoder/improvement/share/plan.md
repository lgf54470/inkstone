...
| H5 | SH-43 | `blog_visits` 无保留期设置（share 已有 `share.visitLogRetentionDays`）：cron 只扫孤儿行，需要 blog settings 段落 + 模态接线，属产品决策 | P2 | ✅ | 待回填 |
...
- 验证：tsc -b 绿；13 项静态门禁全绿（`size:check` 因 `useBlogSettingsModal` 破 50 行拦下一次，按真拆处置——`applySettingsToForm`/`saveSettingsFlow` 改 `...fields` 传参、返回值同样展开，函数回到 39 行、文件 246 行，不刷基线）；vitest 定向 blog 4 文件/11 测试绿，share + shared + worker + blog D1 相关 35 文件/269 测试绿。全量回归 待回填。fix 提交 待回填。
