...
| H8 | SH-46 | share 外的 `Segmented` 仍缺可访问名称（实测 5 处：`blog-dashboard-view/index.tsx:135,272`、`blog-settings-modal.tsx:31,228`、`settings/backup-settings/target-form.tsx:45`）：与 SH-40 同一缺陷族，按铁律14 不在本批夹带；各模块自行接入后把 `tests/share-radiogroup-names.test.ts` 的扫描根从 `features/share` 提到全站 | P2 | ✅ | a75f7e50 |
...
| H10 | SH-48 | 博客中心侧栏底部的三个控件是裸样式控件且两个图标控件只靠 `title` 有名（`features/blog/blog-hub-sidebar/index.tsx:264,271,279`：分类管理 `<button>`、博客设置 `<button>` + `<Settings>`、前台站点 `<a>` + `<ExternalLink>`，全部手写 Tailwind 类串、后两个仅有 `title`）：违铁律10（须走 `components/primitives` 的 `IconButton`）与铁律13（`title` 是 accname 的最后一名，多数 AT 不朗读）；SH-31 在 share 侧已按 `IconButton label=` 收口，此处是 blog 侧同族。SH-46 的真浏览器复核中发现，按铁律14 不夹带 | P2 | 排队 | |
...
## 49 — SH-47（H9）blog 访问日志清理补齐 share 侧两道护栏（2026-09-20）

- 现状与根因：`DELETE /api/blog/visits` 只认 `requireAuth`，两条 share 侧早已立好的护栏在这里都不存在。一是 `older_than` 的 `days` 直查数据库，且 `deleteBlogVisitLogs` 里写着 `Math.max(1, days)`——`?type=older_than&days=0`（或 `-5`、`abc`→NaN）被静默换算成「删掉 1 天前的全部日志」，客户端 SH-43 拦住了 0，但铁律里服务端才是最终信任边界；二是 `type=all` 一次清空该账号全部 `blog_visits`，不要求当前密码，被盗用的会话 cookie 一条请求即可抹掉全部访问痕迹。share 双胞胎分别是 SH-14（400 + 客户端停手）与 SH-12（`requireCurrentPassword` → 401 `wrong_password`）。
- 取舍（对称照搬，不新造机制）：worker 侧直接用 share 用的同一批 lib——`ApiError.badRequest`、`readOptionalJsonValidated` + 新 `blogVisitWipeSchema`（`password` 走 `LIMITS.passwordMaxLength`，不进查询串）、`requireCurrentPassword`（自带 `consumeAttemptBudget`/`assertNotLocked` 节流）；口令校验排在 `deleteBlogVisitLogs` 之前，401 时一行未删。`Math.max(1, days)` 的钳位随之删除：400 已保证 `days >= 1`，留着即不可达死代码（铁律5）。客户端复用同一套流程与文案：`parseBlogCleanDays` 拦 0/NaN → `confirm` → `type==='all'` 时 `promptWipePassword()`（取消即中止）→ `api.blog.cleanVisits(type, days, password)`。demo 后端同步镜像 401，与 share demo 一致地**不**镜像 400（该形态客户端已在上游拦住，两个 demo 双胞胎保持同一不对称）。
- 跨模块共用件（解冻范围内）：`promptWipePassword()` 原先住在 `features/share/share-helpers.ts`，blog 若复用就得静态 import share 的 barrel——把这条 6 行的口令提示移到 `src/client/lib/wipe-password-prompt.ts`（第 2 次出现、约 10 行同形，抽取符合 DRY 门槛），share 两处调用点改指新路径，`share-helpers.ts` 删本地实现与随之无用的 `prompt` import。文案键仍用 `share.verify_password_title`/`share.verify_password_clear_all`/`share.clean_now`：blog 设置模态本就直读十余个 `share.*` 键，键名迁移属另一笔债（SH-34/H 系列），本批不夹带。
- 改动面（16 文件 +323/−40，其中生产 9 文件 +55/−27）：`routes/blog/stats.ts` 加 400 与 401 两道护栏并去掉钳位；`routes/blog/schemas.ts` 新增 `blogVisitWipeSchema`；`lib/api/share.ts` 的 `blog.cleanVisits` 加第三参 `password?` 并只在有值时带 body；`use-blog-settings-modal.ts` 接 `promptWipePassword`；`lib/wipe-password-prompt.ts` 新增；`share-helpers.ts` 删实现、两个 share hook 改 import；`demo/backend/routes/blog-mutations.ts` 的 `registerBlogVisitsRoute(app, data, state)` 加 401 分支、`blog.ts` 调用点传 `state`；`vitest.config.ts` 把新 node 用例登记进 jsdom `exclude` 与 node `include` 两处。
- 测试（红先行）：新 `tests/blog-visit-guards.test.ts`（node + d1-harness，4 例）——`days` 取 `0`/`-5`/`abc` 各 400 且两行日志分毫未动、随后 `days=30` 正常删 1 行；`type=all` 无 body 与错口令各 401（断 `error.code === 'wrong_password'`）且行数不变；传真哈希口令 → 200 `deleted: 2` 清空；`bots` 与 `older_than` 不受重认证影响。首红实测 `Tests 2 failed | 2 passed`（`expected 200 to be 400`、`expected 200 to be 401`）。客户端 `blog-settings-retention.test.ts` 新增 SH-47 describe 3 例（提示恰好一次并把输入的口令带到 API、取消提示则一个请求都不发、清 bot 不提示）+ 新 `demo/blog-smoke.test.ts` 2 例（缺/错口令 401 且 `filterStats.bots` 不变、带对口令 200 且归 0）；demo 路由清单里那两条 `type=all` 探测改带口令 body。SH-43 原有的 `('older_than', 30)` 断言随签名补第三参 `undefined`——顺带把「按天清理不带口令」钉成契约。首红实测 `Tests 3 failed | 7 passed (10)`。
...
## 50 — SH-46（H8）`Segmented` 可访问名称守卫提到全站并补齐 blog 四处（2026-09-20）

- 现状与根因：SH-40 只把 `features/share` 里的 `Segmented` 纳入了守卫（`tests/share-radiogroup-names.test.ts` 的扫描根写死在该目录），同缺陷在别处照旧。本批先把扫描根临时提到 `src/client` 量出真实违规面：**全站 27 处 `Segmented`，无名 5 处**——`blog-dashboard-view/index.tsx:135`（时间范围）、`index.tsx:272`（PV/UV 指标切换）、`blog-settings-modal.tsx:31`（设置分组切换）、`blog-settings-modal.tsx:228`（保留期），以及 `settings/backup-settings/target-form.tsx:45`。其余 22 处（settings 11、share 8、music 3、workspace 2）已全部有名，即「提到全站」不会一次拖出别人台账的债。
- 第 5 处的判定（不改代码，改守卫）：`target-form.tsx:45` 其实已有可访问名称——`Field` 会把 `aria-labelledby` 克隆到自己唯一的合法元素子节点上（`components/form.tsx:396-401`），只是静态扫属性名看不见它。于是守卫加一条**有边界**的豁免：`Field` 且带 `label` 属性且**只有一个有效子元素**时才算被命名。三个条件各自对应 `Field` 的真实行为（子元素是数组时 `isValidElement` 为假、不克隆；无 `label` 时克隆过去的 id 指向空文本），豁免不靠信任：正向由新 `src/client/components/form-segmented.test.ts` 的真实渲染证明（直接子节点 → `aria-label` 为 null 但可访问名称等于 Field 的 label；同一个控件包一层 `div` → 两个属性都没有，即豁免不外溢），边界由守卫自己的用例证明（见下条）。
- 改动面（生产 5 文件，其中 2 新增）：`blog-dashboard-view/index.tsx` 把内联的 `DashboardControls`/`TrendChartCard` 提成 `dashboard-controls.tsx`/`trend-chart-card.tsx`（净 -115 行）——动机是**可测**：命名断言必须能单独渲染这两个控件，内联在 index 里就只能整页渲染；顺带把 `RealVisitorsToggle` 从 `DashboardControls` 拆出以满足 50 行上限。四处命名：时间范围 `label={t('blog.range_label')}`、指标切换 `useId()` + `<h3 id>` + `aria-labelledby`（控件旁就有可见标题，走 labelledby 而不是再编一个 aria-label）、设置分组 `label={t('blog.settings_tab_label')}`、`RetentionField` 的可见标签 span 补 `id={labelId}` 并让分段控件 `aria-labelledby={labelId}`。新文案键 2 个（`blog.range_label`、`blog.settings_tab_label`）双语言同步。
- 守卫（改名 + 提到全站）：`git mv tests/share-radiogroup-names.test.ts tests/radiogroup-names.test.ts`，`CLIENT_DIR = path.join('src','client')`，`describe` 回调重命名为「every client radiogroup is named (SH-40, SH-46)」。为让豁免可证，把 `scanFile` 拆成 `scanSource(file, source)` + 读盘薄壳，新增第 3 条用例扫一段内联 fixture（7 个站点、5 处应报）：单子带 label 的 `Field` 豁免、双子 `Field` 的两个都报、无 label 的 `Field` 报、`label='c'` 通过、**`aria-label='d'` 仍报**（`Segmented` 根本没有该 prop，写了不会落到 `aria-label` 上）、裸控件报。首红即由这条 fixture 用例查出我最初的错误假设——守卫拒绝 `aria-label` 是对的，是我的夹具写错了。
- 测试（红先行）：新 `blog-dashboard-view/radiogroup-names.test.ts` 2 例（范围控件带 `aria-label` 且不被 labelledby 双重命名；指标控件的可访问名称等于**卡片标题**而不是它下面那行说明）+ `form-segmented.test.ts` 2 例 + 守卫 1 例；`blog-settings-retention.test.ts` 补 SH-46 describe 2 例（保留期分段控件的可访问名称等于其可见标签文本、无标题可指的分组切换器带自己的 `label`）。断言两侧都走 `t()`，因为该 harness 不加载语言包（`t()` 回显键名）。
- 变异 8 发全杀（/tmp/mutH8 备份 4 文件、进程内逐字节校验还原 clean）：M1 删范围控件 `label`、M2 **把 `aria-labelledby` 从 `<h3>` 改指到说明 `<p>`**、M3 删 `RetentionField` 的 `aria-labelledby`、M4 删分组切换器的 `label`、M5 扫描根退回 `features/share`、M6 豁免去掉「唯一子元素」条件、M7 豁免去掉「带 label」条件、M8 让 `aria-label` 也算有名。**M2 只有行为断言杀得住**（静态守卫只看属性在不在，看不见指错目标），这一发就是「守卫广覆盖 + 行为读渲染结果」两层设计存在的理由；M5 由「站点数 ≥ 12」那例杀掉（实测 27），M6/M7/M8 全由 fixture 那例杀掉。
- 踩坑三条：①新行为文件最初在用例**末尾**手调 `unmount()`，M1 首跑报了 3 个失败——第一例断言先失败就跳过了卸载，残留在 `document` 里的旧 radiogroup 让下一例的 `querySelector` 读到 stale 节点（与 SH-43 记过的级联污染同一形态）。改为模块级 `mounts[]` + `afterEach` 排空后，同一变异精确杀掉 1 例；**归因可疑时先怀疑测试脚手架**，否则这条证据是脏的。②夹具最初把 `aria-label='c'` 当成「已命名」，是守卫纠正了我：`Segmented` 的 prop 叫 `label`，它自己渲染出 `aria-label`。③`style:check` 拒绝「可转单引号的双引号串」，夹具的字符串数组写法被拒 8 处；改模板字面量（行号与原数组逐行一致，`fixture.tsx:5` 这类断言不用动）。另外 `RANGE_OPTIONS` 提到组件内是 `i18n:check` 的硬要求（模块作用域 `t()` 会冻结初始语言），与 `share-dashboard-view.tsx:66` 同形。
- 局限（如实登记）：未跑真实浏览器（`e2e-visual`/`check-contrast` 都不开博客看板与博客设置模态），命名正确性由 jsdom + 静态守卫两层把守；守卫判的是「有没有命名机制」，**不判名称文本是否有意义**（把 `label` 指向另一个已存在键只有那两条具名断言能杀，本批正是靠它们）；豁免的「不外溢」只测了 `div` 包一层这一种反例，`Field` 里再嵌一层容器（`<Field label><div><Segmented/></div></Field>`）目前全仓不存在，若将来出现会被守卫放行而实际无名——新增该形态时按同一手法补例。`RetentionField` 沿用的 `share.retention_days_label` 等跨模块文案键仍属 SH-34/H 系列的迁移债，本批不夹带。
- 验证：tsc -b 绿；13 项静态门禁全绿（`style:check` 曾被夹具的双引号拦一次，见踩坑③）；vitest 定向 blog + share + components + 守卫 40 文件/189 测试绿（pre-commit 钩子另跑相关 120 文件/790 测试绿）。全量回归 249 文件/1928 测试绿（REGRESSION_EXIT=0，按本文件回归标准串行跑 `--no-file-parallelism --testTimeout=30000`，516.68s；较 SH-47 轮次 247/1921 多 2 文件/7 例，即 form-segmented 2 例 + 看板 radiogroup-names 2 例 + 守卫夹具 1 例 + 模态 SH-46 describe 2 例）。**快照坑一条**：`git archive HEAD | tar -x` 出来的 detached 目录里 `blog-frontend/node_modules` 不在场（它是未跟踪软链），`tests/markdown-renderer-parity.test.ts` 会因 `astro/tsconfigs/strict` 解析不到而整文件加载失败——第一跑因此报 `1 failed | 248 passed`、`Tests 1927 passed`（**没有一条测试失败，纯环境缺件**）。补软链后第二跑才是真绿。教训：detached 快照必须把两个 `node_modules` 软链都接上，且看到「文件级失败 + 零测试失败」先按环境问题查。fix 提交 a75f7e50。

## 51 — SH-46 真浏览器复核（关闭 H 系列共同的「未跑真实浏览器」局限）＋ 新登记 SH-48（2026-09-20）

- 为什么补这一节：H1…H9 每一节的「局限」都写着同一句话——`e2e-visual`/`check-contrast` 都不打开博客看板与博客设置模态，所以命名只由 jsdom 断言把守，没在真实浏览器读一遍。用户起了本机实例（worktree 内 `npm run dev`，7712 被占所以落在 **7713**）并注册了账号，这条局限第一次可以被实测关闭，遂按人打开的路径走一遍并读无障碍名。
- 路径与读数（全部只读，未点任何清理/删除）：登录 → 侧栏 `aria-label="博客管理中心"` 按钮 → 仪表盘 → 模态「博客设置」→「流量过滤设置」页。真实 DOM 里四个 `role=radiogroup` 的**可访问名称来源**分别是：①区间选择器 `aria-label="分析时间范围"`（= 新键 `blog.range_label` 的 zh 值，子项 `24h/7d/30d/全部`）；②指标选择器 `aria-labelledby=_r_b_` → 目标节点文本「访问流量趋势」（= `blog.timeline_trend_title`，**不是**它下面那行说明，正是 M2 变异要杀的那件事）；③设置分组切换器 `aria-label="博客设置分组"`（= `blog.settings_tab_label`）；④保留期 `aria-labelledby` → `<span>` 文本「自动保留天数」（`useId` 生成的 `_r_1n_`，目标存在且文本精确）。**结论：SH-46 四处命名在真实浏览器里全部成立，zh 语言下名称即预期文案。**
- 两条顺带核实（都不构成缺陷，记下来免得下轮重新怀疑）：①保留期那段可见文案住在普通 `<span>` 而非 `<label>`——这是**正确**做法，`div[role=radiogroup]` 不是 labelable 元素，`<label for>` 指不过去，`aria-labelledby` 按 accname 可指向任意元素子树，share 侧 SH-40 同形；②控件的选中态由 `role=radio` + `aria-checked` 真实携带（当前值 30d 为 `true` 且 `tabindex=0`，其余 `-1`），说明分段控件的 radio 语义没被命名改动碰坏。
- 一处真发现 → **新登记 H10/SH-48**（铁律14，不在本批动）：进入模态时点的那个「博客设置」按钮在真实 DOM 里是 `title="博客设置"`、`aria-label: null`、无文本，查下来是 `features/blog/blog-hub-sidebar/index.tsx:271-278` 手写的裸 `<button>`（同处还有 `:264` 的分类管理 `<button>` 与 `:279` 的前台站点 `<a>`，三个都自带一整串 Tailwind 而没走 `components/primitives`）。违铁律10 与铁律13（`title` 是 accname 算法的最后一名，多数 AT 不朗读）；`IconButton` 本身是对的（`primitives.tsx:126` 把 `label` 落成 `aria-label`），只是这里绕过了它。share 侧同类早在 SH-31 就收口成 `IconButton label=`，故属 blog 侧同族欠账，按模块自行清偿。
- 顺带一条环境结论（与代码无关，值得记）：worktree 的 `node_modules` 是指向主检出 `/home/kubuntu/code/cloudflare/inkstone/node_modules` 的**软链**，Vite 解析到 realpath 后超出默认 `server.fs.allow`（只放行 worktree 根与 `vite/dist/client`），于是 `@fontsource-variable/inter` 的 woff2 报 403、字体静默回落系统字体，且两个检出抢同一份 optimizer 缓存（表现为 "Re-optimizing dependencies because vite config has changed"）。**这不是本分支引入的缺陷**，页面功能不受影响；真要修是给 `vite.config.ts` 的 `server.fs.allow` 补 realpath，属另一分支另一批。浏览器控制台里 `content.js` 抛的 `{httpStatus: 200, code: 403}` 是某个扩展的内容脚本，与应用无关。
- 复核性质说明：本节是**验证与登记**，无生产代码改动，因此不跑变异、不跑全量回归（工作区与 a75f7e50 一致，仅本文件变更）。

## 52 — 修 §51 登记的环境缺陷：worktree 里 dev 服务器把 node_modules 字体判到放行清单外（2026-09-20）

- 触发：用户读了 §51 那条环境结论后要求修（"fix bug"，附控制台 403 与两条 `code: 403` 的 promise rejection）。
- 根因（不起浏览器即可复现）：worktree 的 `node_modules` 是指向主检出的软链，Vite 把 `node_modules` 里的资源按 **realpath** 出 URL 并只放行 `server.fs.allow` 下的路径。实测默认清单就是 worktree 根（`searchForWorkspaceRoot(cwd)` → `/home/kubuntu/code/cloudflare/inkstone-share-improvement-qoder-qwen38f`），而 realpath 是 `/home/kubuntu/code/cloudflare/inkstone/node_modules`，于是一发 403 的是**整类**字体，不止 §51 看到的 Inter：`@fontsource-variable/inter` 的 woff2、`katex/dist/fonts/KaTeX_Math-Italic.woff2`、`KaTeX_Main-Regular.woff2` 三支全 403（`mimeType=text/html` 即 Vite 的拒绝页）。表现是字体静默回落系统字体，功能不坏，所以只有控制台与视觉受影响。
- 诊断的确切边界（同一份未打补丁的实例上实测对照）：`/@fs/<主检出>/node_modules/katex/dist/katex.min.css` **本来就 200**，而同一目录树下的 `.woff2` 403。差别不在路径而在管线——CSS/JS 走 transform 中间件（按模块 id 取文件），字体是 CSS `url()` 重写出来的 `/@fs/` 静态请求，只有后者吃 `server.fs.allow`。所以这一行放行的正是「按 URL 直出的静态资源」，不是整个依赖树。
- 改动（1 文件 +13 行）：`vite.config.ts` 新增 `servedFsRoots()`，仅当 `node_modules` 自身是软链时把它的真实目录追加进 `server.fs.allow`，否则原样返回项目根——非 worktree 的正常检出、以及尚未 `npm ci` 的检出（用 `lstatSync(..., { throwIfNoEntry: false })` 判空，不留空 catch）拿到的清单与 Vite 默认等价，不放宽任何东西。
- 安全边界（放行清单的改动必须反向断言，否则「修好了」等于「关掉了守门人」）：补丁后的实例上 `/@fs/<主检出>/vite.config.ts` 仍 403、`/@fs/home/kubuntu/.ssh/id_ed25519` 仍 403，而三支字体从 403 变 200。即放行的只有依赖目录，不含主检出的源码与凭据文件。另澄清一条看着可疑但**与本改动无关**的读数：`/@fs/<主检出>/.dev.vars` 回 307，`Location` 是把 `@` 重新编码成 `%40` 的同一 URL，跟随后落到 SPA 外壳（`text/html`，与 `index.html` 同级字节数），不是密钥文件；这条 307 在**打补丁之前**的实例上实测同样存在（7712 未重启的旧进程与 7799 补丁后进程各测一次，均为 307），属 Vite 既有行为。
- 变异 1 发即杀（/tmp 备份 + 逐字节校验还原）：M1 删掉 `fs: { allow: servedFsRoots() }` 这一行 → 字体回到 403；还原 → 200。这一行是承重件，`servedFsRoots()` 本身不是。
- 顺带把 §51 那句「扩展噪声」升级为实测：干净浏览器（无扩展）打开同一 URL，console 只有 vite 连接与 React DevTools 提示三条，没有 `{name:'n', httpStatus:200, code:403}`；全仓（含 `node_modules`）也搜不到 `httpStatusText`/`httpError` 这两个字段名。那两条 rejection 确定来自用户浏览器扩展的内容脚本，不登记为应用缺陷。
