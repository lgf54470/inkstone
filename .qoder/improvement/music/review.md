...
- 队列拖拽重排/插入指定位置（M）；专辑/艺人分组视图（L，「像音乐 app」的分水岭，字段已入库后端零改动）；歌单分享/智能歌单/歌单封面（M–L）；重复文件检测（M–L，需 hash 列 + 回填，建议只对新增算）；gapless/crossfade/音量归一化/EQ（M–XL，暂缓）；睡眠「播完当前曲停」（S）；在线歌词搜索（M，可照抄 cover-lookup 代理范式，但需先定版权边界）；整库纳入备份体系（M，误删不可恢复的风险真实但低频）。◐
  ...
  - ✅（M-55b4）博客前台播放器按 mime 选元素并给出画面：博客的 `normalizeMusicTrack` 是白名单式投影，权威 mime 在进前台的路上就被丢掉，播放器又固定 `new Audio()`，于是发布到博客曲库的片段「只有声、没有画」——而 `<audio>` 加载带视频轨的容器在部分浏览器里连声音都不给。修成三层各管一件事：数据面补 `mime`（`asString(row.mime)`，缺失读成空串按音频走），判定面新增 `src/lib/music-media.ts`（与主应用 `src/shared/music-media.ts` 同规则的**镜像**——blog 是独立 Astro 工程、无 `@shared` 别名，跨仓库引用会把两个应用的依赖图焊在一起，代价是两处规则要同步改），引擎面把「一只元素」换成 `elementsByKind`（每类一只）+ `mediaElement`（当前活动那只）。切换次序必须是**先交接再退休**：`wireMediaEvents` 的每个监听都带 `element === mediaElement` 门槛，若先退休，旧元素自己发出的 `pause` 会在交接窗口里把播放态写成暂停（变异「监听无门槛」与「先退休」各杀一条断言）。频谱节点从全局一只改为按元素 `WeakMap`——一只媒体元素只能接入音频图一次，跨元素复用会让视频轨静默沿用音频链（读到的仍是上一类的数据），可视化取节点的 effect 因此把依赖从 `[playing]` 补成 `[playing, currentId]`。画面面新增 `MusicVideoStage.tsx`：展开卡片借出一只 16:9 盒子（`claimVideoStage(host)` 返回释放函数，收起即归还），音频轨根本不渲染盒子；元素由 store 持有、由 CSS 铺满（`> video` + `object-fit: contain`），因为 React 一旦持有它就会被断点重建（与 M-55b2 同一个理由）。不给 `controls`：卡片自带的语义传输控件已是这个元素唯一需要的操作面，再叠一层原生控件会有两套互不相识的播放按钮。`playsInline` 是移动端唯一让片段留在页面里而不是跳去系统全屏播放器的开关。
    - 已知限制：**博客前台没有任何浏览器门禁**（CI 的 blog 任务只跑 typecheck/lint/test/build 与 `blog-e2e-smoke.mjs`，后者只 ping 端点），所以展开卡片里那只画面至今只有 jsdom 证据——不解码字节、不排版，铺满与 contain 是否真的好看未经像素验证，已登记 M-56（该门禁要先裁决：真片段夹具进仓库还是 CI 装 ffmpeg）。本次另对运行中的 Worker 做过一次真实 HTTP 核对——博客公开投影里的曲目带 `mime: "video/mp4"`，其 streamUrl 在 Range 下回 206 + `video/mp4` + `Accept-Ranges: bytes`，所以未证的只剩浏览器里的解码与排版，不再是「数据到没到博客」；收起徽标、音乐中心弹窗仍显示封面（片段在这两处只有声音，与主应用 M-55b2 的范围裁决一致）；博客舞台是单槽而非主应用那一栈（前台同一时刻只存在一张展开卡片，没有嵌套归还顺序要处理）；`astro check` 在本仓库基线就有 2 条错误（未改动的 `src/middleware.ts` 里 `HTMLRewriter` 未声明），本项新增 0 条，但这条基线意味着「typecheck 绿」在 blog 侧不可声明；`isVideoMime` 与主应用同规则却两份实现，规则改动必须两处同步（镜像的取舍已写在文件注释里）。
    - 过程教训（供后续项复用）：jsdom 里 `document.body.innerHTML = ''` 会把 store 的模块级单例元素变成**游离节点**——下一曲 `playTrack` 又把它挂回 body，于是「parentElement 是 body」这类断言在错误的代次上假绿。引擎测试走 `vi.resetModules()` + 每例 `await import('./music-player')`（与既有 `music-queue-seed.test.ts` 同法），组件测试不擦 body、而把断言限定在 `.music-video-stage > video`，不去断言「文档里没有别的 `<video>`」。`size:check:blog` 把 `describe` 回调一起按 50 行计，新测试文件第一次就因 54 行的 describe 被拒——拆成两个 describe（按「选元素」与「退役元素各管自己」分组）而不是往基线里塞 grandfather 条目。`pause()` 桩必须像浏览器那样把事件发出去（`dispatchEvent(new Event('pause'))`），否则「先交接再退休」这条次序不变量在测试里根本不存在，变异也就杀不掉。**「先退休再交接」这个变异第一次存活**：`playTrack` 在换元素之后显式写 `playing: Boolean(element)`，把退休窗口里那次误写的 `playing:false` 盖掉了——要暴露它必须走不经过 `playTrack` 的换类路径（刷新曲库让同一首换 mime，再调 `setVolume` 这类只取元素、不改播放态的入口）。补这条用例时又踩了模块代次：`clearApiMemoryCache()` 静态 import 的是 `resetModules` 之前那一代的 api，第二次 `loadMusicLibrary(true)` 于是仍拿旧投影，helper 里必须 `await import('../../lib/api')`。最后一条最贵：**收尾时为了回退变异残留，对已跟踪且带未提交改动的文件跑了 `git checkout -- <file>`，把整份未提交的 `music-player.ts` 退回了 HEAD**。变异脚本本来就只该用它自己的字符串还原（它其余 13 例都这么做且都成功）；恢复靠会话记录里逐条重放 Edit、再补回被脚本批量做的 `ensureAudio`→`activeMedia` 重命名（8 处），最终以「14 例变异全杀 + 47 例音乐测试全绿 + blog 全量 292 例」确认与原实现一致。回退未提交的改动只能用精确字符串还原，不能对跟踪文件用 checkout/reset。
