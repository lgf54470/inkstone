...
| 2026-09-19 | M-15 沉浸式播放器补可见关闭按钮与对话框可访问名（UI-3） | 本次提交（hash 由下一次提交回填） | 新增 2 例先红后绿（dialog 曾挂通用 overlay.dialog 且无可点关闭→现 aria-label=music.immersive 且 exit_immersive 按钮点击调用 onClose 一次；修复前断言实测收到 'overlay.dialog'），hub modal 同法补 ariaLabel=music.hub_title（其关闭按钮原本已在），typecheck ✅ |
