...
- [ ] M-28b PERF-15 歌单 addItem 单批写入 / reorder 收敛往返；客户端批量 items 接口
...
| 2026-09-19 | M-28a PATCH track 去重复行读 + tag 归属校验并入 batch（PERF-14） | 本次提交（hash 由下一次提交回填） | 1 例先红后绿（新增 recordingDb 代理统计 prepare 的 SQL：track 行 SELECT 3→2 即写前存在性读+响应权威读、外键归属独立探测 1→0；同一 PATCH 拒绝外来 tag 的行为断言 tagIds==[own] 保持）。tracks.ts：registerPatchRoute 删除第一次仅判存在的 loadTrackRow，row 复用给 coverDataUrl 分支；replaceTrackTags 由「全量 owned select + 逐 tag INSERT」改为 DELETE + INSERT…SELECT（user_id 双保险）单 batch。M-28 行按原子提交拆分为 M-28a/b/c 三行（PATCH 去重 / 歌单批量 / transfers 任务模型为三件独立事项）。music-routes 32 例全绿，全量单测见本行提交结果，typecheck 与静态门禁 ✅ |
