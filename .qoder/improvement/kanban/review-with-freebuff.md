...
### K-30（用户报告，本轮修）无标签卡片的悬停行压在标题上 → 已修
- 现象：卡片初始没有标签时，鼠标悬停在卡片上会显出「标签」按钮，它与卡片标题重叠，完全看不清楚。
- 证据（在 `:7712` 上自建一块含「无标签卡」的板实测）：看板覆盖层里无标签卡的悬停行与标题同起点——行 `absolute inset-x-3 top-3`（`38,157 244×22`）与标题（`38,157 244×19`）重合，「添加标签」按钮压在其上 `42×19`、详情钮 `14×14`、复选框 `14×14`；同板上有标签的卡（行在流内 `38,210`、标题 `38,240`）无重叠；笔记内联预览看不出重叠，因为 `.ink-prose h3` 的 `26.4px` 上边距正好把标题推低——**覆盖层把卡片画在笔记之外**，prose 的边距不再掩盖它，这正是报告发生在全屏看板的原因。第三条证据由 axe 给出：那条 `color-contrast` 的 `bgOverlap` 恰好落在该卡标题的 `span` 上（标题写在什么底色上判不出来）。
- 根因：`kanban-card.tsx` 的 `headerOverlayClass()`——无标签时把整行拿出流、绝对定位到卡片顶边，图的是「不占一行的高度」。画廊的 `GalleryTagsHeader` 是同一写法（无标签即 `absolute left-3 top-3`），在没有封面的瓦片上同样压住标题。
- 修法：① 卡片行永远留在流内（删掉 `headerOverlayClass` 与 `overlayClass` 这个 prop，`kanban-card-header.tsx` 不再接收）；② 画廊行只允许浮在**真封面**上（`galleryCoverUrl()` 抽出来给封面与行共用，`floatsOverCover` 为真才绝对定位），无封面时回到流内。行内留位换来三件事：没有东西压住标题、悬停不再推动指针下的内容、无标签卡与有标签卡等高。
- 验证（先红后绿）：门禁侧先写复现——fixture 新增无标签卡 `gate-c`，`KANBAN_FIXTURE` 改为同时要求三张卡（否则上一轮留下的旧板会被当成自己那块，量不到无标签卡）；新增 `assertKanbanRevealRows`，在**笔记内、看板视图、画廊视图**各量一遍「悬停控件与标题矩形是否相交」，控件按 `group-hover/card:opacity-100` 这个「谁被悬停显出来」的标记找（与组件内部结构无关，改结构不会漏测）；悬停只改 `opacity`，所以量的是几何，headless 下 `@media (hover: hover)` 不成立也跑得动。复现跑红 3 条：看板视图 `62×19`、画廊 `14×4`，以及原有的「axe 只该把标签芯片交给审查者」被新卡标题的 `bgOverlap` 打破。修后 `e2e-visual` **243 passed / 0 failed**；同一量测修前/修后：覆盖层无标签卡 `270×45` → `270×75`（与有标签卡等高），行 `38,157 244×22` 由 `absolute` 变 `static`，标题 `38,157` → `38,187`，三处重叠归零。
- 单元级：`kanban-card.test.ts` 原先把「无标签就浮起来、不占行」当成规格（两条用例），现改为断言新结构（无标签卡的行在流内且不含标题；画廊只在有封面时浮起），并注明像素真相在浏览器门禁。
- 变异自检：把绝对定位那一版改回去（卡片行 + 画廊条件）恰好杀对应两条用例；`/tmp` 备份原样还原（字节相同）。
- 局限：无标签卡的高度变了（笔记内 `270×84` → 约 `115`、覆盖层 `45` → `75`）——这是为「不压住内容」付的价，且只是追平有标签卡；笔记里标题的 prose 样式漏入由 K-31 结案（本轮已修）。

### K-31（用户本轮指定）prose 对块内元素的排版主张漏进看板 → 已修
- 证据（在 `:7712` 上自建板实测，逐元素比对「笔记内联」与「全屏覆盖层」两处渲染）：卡片标题在笔记里 `18.24px/650/23.35px/-0.3466px/margin 26.448px 0 8.0256px`，在覆盖层里 `14px/600/19.25px/normal/margin 0`；描述 `margin-bottom 8.64px` vs `0`、且覆盖层有 `mt-1` 的 `4px` 而笔记没有；卡片选择框在笔记里被画成 prose task-list 皮肤（`appearance: none`、`16.8×16.8`、`1px` 边框、`--r-5` 圆角、`--bg-inset` 底）而覆盖层是自己的 `appearance: auto`、`14×14`；凡是靠继承取字号的元素（徽标、`svg`、`path`）在笔记里继承 `16px/26.4px/-0.08px`、在覆盖层继承 `13px/20.15px/normal`；画廊封面在笔记里被 prose 的 `img { height: auto }` 顶掉 `h-full`。
- 根因：块在笔记里画在 prose **之内**、在它自己的全屏里画在 prose **之外**，而 prose 是按元素声称的（`h1`–`h6`/`p`/`ul`/`ol`/`li`/`img`/`input[type=checkbox]`…）且**未进层**——未进层的作者样式胜过任何层内规则，所以写在元素自身上的 utility（Tailwind utilities 在 `@layer utilities`）在笔记里一律输给 prose，于是「同一元素在两边不同」的全部原因都来自：笔记多了一层 prose，且它按元素赢过 utility。
- 修法（本模块自持，不动 prose 对笔记本身的作用域）：① `.kanban-canvas` 声明看板自己的基准字型（`--font-ui`/`--text-13`/`--ui-line`/`letter-spacing: normal`），canvas 即「笔记挂载、覆盖层借用」的那一个元素，两处因此继承同一个基准；`--ui-line` 是新加的令牌（`1.55`，body 与看板共用，私有令牌不影响 `tokens:check` 漂移基线）。② `.ink-prose [data-kanban-canvas]` 内把 prose 对块自身标记的元素主张交还（标题 margin/字号/字重/行高/字距/`position`、段落 margin 与 `text-wrap`、列表 margin/padding/`list-style`、`li` margin、`img` 装饰），交还值即 preflight；canvas 作用域使块旁的快照（`.kanban-snapshot`）不受影响，它本来就该用 prose 列表样式。③ 标题的字号字重改放到包裹元素（笔记对包裹无规则）——`kanban-card.tsx`/`kanban-gallery-view.tsx`/`kanban-empty-board.tsx`/`kanban-calendar-view.tsx`/`kanban-comments.tsx`/`kanban-item-detail-description.tsx`/`kanban-item-detail-fields.tsx` 共 8 处；卡片标题与描述间距改由包裹 `gap-1` 提供，描述去掉自己的 `mt-1`。④ 画廊封面（canvas 内唯一的 `img`，prose 的 `height: auto` 是 utility 拿不回的那一类）的盒子改由 `.kanban-cover` 声明，组件里删掉 `h-full`。⑤ 复选框：prose 的 task-list 皮肤本属任务列表，改为 `.ink-prose .contains-task-list input[type=checkbox]` 作用域（`prose/lists.css` 六条），看板自带的 10 个复选框随之回到自己的原生盒子，不必按尺寸在 CSS 里重述（`size-3`/`size-3.5` 两种尺寸）。
- 验证（先量后修、修后再量）：一次性探针在同一实例上把**八个视图**逐个在笔记与覆盖层各读一遍、按元素路径逐属性比对：修前 `board 139/139 元素 139 条差异`、`table 323/323 → 1`、`calendar 288/288 → 102`；修后 `board 0`、`table 0`、`calendar 0`、`timeline 0`、`gantt 0`、`list 0`、`gallery 0`（唯一残留是盒尺寸与解析后的 `grid-template-columns`，那是两个不同大小的面，已从比对属性里剔除）。
- 门禁化（不再靠一次性探针）：`scripts/e2e-visual.mjs` 新增 `assertKanbanCanvasBase`（断言 canvas 的基准字型等于 `body` 的，而非笔记的，**身份不写死数字**）、`readKanbanViewsInline`（打开覆盖层前把八个视图逐个画在笔记里，按元素路径记下排版）、`readKanbanViewStyles`/`assertKanbanViewParity`（覆盖层里同样读一遍再逐元素比对，只比决定「怎么排字」的属性），并把 `assertKanbanViews` 的视图扫描与它合流（同一次扫描既验证内容又取样比对）；`pressKanbanView` 现在**验证按压落点**（见下条）。
- 顺带修正的门禁脆弱点：笔记 pane 的滚动区伸到 app 固定底栏（音乐控件）之下，视图标签可能被测到那一块后面——一次真实按压落在那里会去操作音乐控件；现在按 `elementFromPoint` 确认指针确实在该标签上（必要时换 `block: start/center` 重试），落不到就报「谁在上面」而不是照点。
- 变异自检（实跑一门禁，非推断）：把标题的交还块（`:is(h1..h6)`）临时删掉 → `e2e-visual` **249 passed / 3 failed**，恰好是画标题的三个视图，且报的正是机制：`board` `h3 "Gate first task" :: font-size 15.96px → 14px | font-weight 650 → 600 | line-height 20.4288px → 19.25px | letter-spacing -0.30324px → normal | margin-top 23.142px → 0px | margin-bottom 7.0224px → 0px | position relative → static`，`calendar` `h3 "2026 - 09" :: 17.1px → 15px`（同组属性），`gallery` 同 `board`；不画标题的 table/chart/timeline/gantt/list 仍绿。数字与机制对得上：`15.96px = 1.14em × 包裹的 14px`、`23.142px = 1.65 × 14px`、`-0.30324px = -0.019em × 15.96px`，即删掉交还后 prose 的 `h3` 又赢了包裹；`cp` 备份恢复后字节相同，重跑静态门禁与 `comments:check` 全绿。
- 未动（如实声明）：`pre`/`code` 在 canvas 内的 prose 主张未交还（看板自身只在错误兜底里画它们，数量级可忽略且现有 `.ink-prose .kanban-error pre` 已交还其装饰）；`.kanban-snapshot` 有意保留 prose 列表样式；两者都不由门禁断言。
...
- 已更新（2026-09-23 后续专项，K-26）：`check-contrast.mjs` 不再假设「遮挡者是参考卡」也不再把同一实例读两遍，改由**表面自声明**判不了的项（axe `messageKey` + 目标形状 + id 三重命中），规则与分类器抽到 `scripts/lib/axe-review.mjs`、由新增 `tests/axe-review.test.ts` 4 例钉住（三重各自承重 + 全局三类不入规则 + 同文案不同 target 仍失败），`e2e-harness.mjs` 的 `runAxe` 摘要补 `key`，`AGENTS.md` 同段改写。**`check-contrast` 现已 exit=0**（两套主题各 23 项 axe 检查、层级与 7 强调色量测 0 低于 AA；各 1 条 `color-contrast ×2 (bgOverlap)` 按名放行并打印理由）；全量 `test:unit` 290 文件 2704 测试 ✅、typecheck 与 14 项静态门禁 ✅。只剩 `e2e.mjs` 的 L-04 一条遗留；两侧变异自检（门禁换 key、测试侧三次变异）均被杀且备份原样还原。
- 已更新（2026-09-23，K-31 专项）：`e2e-visual.mjs` **252 passed / 0 failed**（新增 8 条「视图在笔记与覆盖层排版一致」+ 1 条「canvas 基准字型等于 body」+ 1 条「笔记里确有此块」）；`typecheck` ✅；全量 `test:unit` 290 文件 **2705** 测试 ✅；14 项静态门禁 ✅（`comments:check` 白名单重生成 689 文件 / 4743 条）；变异自检一次（删掉标题交还块 → 249/3，报的正是 `board`/`calendar`/`gallery` 三个画标题的视图），恢复后字节相同。同一轮曾有一次 `e2e-visual` 在**幻灯片**场景提前失败（`svg=0,katex=0,charts=0` 后找不到「显示幻灯片列表」控件）；重跑同一修订即 252/0，判定为本机 run 间偶发（与本轮改动面无关，亦未留下可复现的痕迹），如实记录不归因。
