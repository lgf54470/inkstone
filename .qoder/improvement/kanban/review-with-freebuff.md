...
- 局限：无标签卡的高度变了（笔记内 `270×84` → 约 `115`、覆盖层 `45` → `75`）——这是为「不压住内容」付的价，且只是追平有标签卡；笔记里标题的 prose 样式漏入见 K-31（本项未动）。

### K-31（本轮新增同模块，未修）prose 的 h3 样式漏进卡片标题 → 登记
- 证据（同一次实测）：笔记内联预览里卡片标题是 `font-size: 18.24px`、`margin: 26.448px 0 8.0256px`，而卡片自己声明的是 `text-[length:var(--text-14)]`；覆盖层（不在 `.ink-prose` 内）同元素为 `14px` / `margin: 0`。即 `.ink-prose h3` 的排版规则越过了富媒体块的边界，把卡片标题按正文小标题渲染。
- 影响：预览里的卡片比设计高且字更大，同一块板在笔记内与覆盖层里两种长相；K-30 的修复在预览里也因此多出 `26.4px` 的额外留白。
- 未修原因：修它要动 `.ink-prose` 对块内标题的作用域（改为 `.ink-prose .kanban-block h3 { margin: 0; font-size: inherit }` 一类），会同时改变**有标签**卡片的现有长相，属于跨表面的排版变更，需与 K2-05 一类视觉门禁一起判定；按规则 14 不夹带。
