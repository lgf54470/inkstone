...
- [x] KU-31 头部每次 render 的 `kanbanActiveItems` / `countTags` / `people` 全量重算 —— 真凶只有 `activeItems`（render 体内造新数组，正是标签条 `countTags` 的 memo 键）；`people` 已按 `[data]` 记忆且那是**正确的粒度**（新卡就是新的人），不改
- [x] KU-32 列 memo 被逐 render 新建的对象击穿（K-19 只兑现到卡片层）—— 计划的归因只对一半：`dnd` bundle 的成员本来就各有身份，「记忆 bundle」**不是承重件**（变异实测仍绿，故 `kanban-board-dnd.ts` 未改），坏在 bundle 连同 `cell` 对象被 `{...props}` 透传给 memo 的列，另有 `group`/`selectAll`/`onCollapse`/`toggleCollapse` 四个写入者每次重画都换新
- [ ] KU-33 渲染预算测试（500 卡 × 8 视图）+ 真机量测脚本
- [x] KU-34 写回成本复测（K-20 已裁定不取自适应静默期；KU-16 后复测视图类提交是否仍需写回）—— 结论：查视与编辑共享同一个 500ms 静默期（commit 收尾一律 `scheduleKanbanWrite`），突发合并与逐次落盘已各有承重用例，故按 K-15 口径保留整篇写回；**未改代码、未新增测试**（理由在进度日志里）

### 批次 5 · 安全

- [x] KU-41 文本附件预览的跨源 `fetch` 走外部资源策略 + `AbortController`/超时 —— 策略直向：命中时**不挂载发起请求的组件**，所以没有请求可取消；并补 10s 截止时间（到期走同一失败态与重试）与卸载/重试时取消
- [x] KU-42 分享/导入来源的 description 预览注入回归（把「宿主渲染器即净化器」钉成门禁）—— 用真实预览管线走一条敌意 description（script/iframe/onerror/`javascript:` 链接），断言进不了面板且正文照旧渲染
- [x] KU-43 护栏：栅栏来源的 URL 字段必须过 `safeKanbanUrl` —— **与计划书不同**：真实护栏不在渲染面而在**解析边界**（`assertFenceUrlsAreSafe` 把非法协议整条栅栏打成错误态，而不是静默丢字段），因此新门禁改成「读 `types.ts` 找所有名字像 URL 的字段 + 逐个喂敌意/合法取值」，新增字段漏登即红；两条守卫并入一个提交，理由在进度日志里
- [~] 两条守卫原本分属两个条目，实际**合并为一个提交**（KU-42+KU-43）：它们同类（都是把已有前提钉成测试）、共享同一个 `scripts/check-comments.mjs` 白名单，拆开会让中间快照的白名单与树互相矛盾
...
| 日期 | 条目 | commit | 回归结果 |
| --- | --- | --- | --- |
| 2026-09-24 | KU-34 写回成本复测：查视提交是否仍需整篇写回 | （本提交） | **结论：按 K-15 的持久化口径保留，不改代码也不新增测试。** 三条复核：① 提交路径与提交种类无关——`registry.ts:265` 的提交收尾一律是 `scheduleKanbanWrite(entry, rerender)`，查视与编辑共享同一个 500ms 静默期；② 突发合并已被 K-20 的计数用例钉住（滑杆扫 12 步 = 1 次写、相隔 700ms 的 6 个手势 = 6 次），而那组用例只驱动 `scheduleKanbanWrite`、并不读 kind，故对查视同样成立；③ 每次查视确实落进文档由 KU-16 的端到端用例钉住（13 次查视逐次写回、撤销仍回到上次编辑）。复核实跑：`write.test.ts` 11 例 + `kanban-lookup-history.test.ts` 4 例 ✅。**未新增测试的理由**：本条要的两半已各有一处承重用例（一处管合并、一处管落盘），再加一处只会断言两个已钉事实的合成。**未动代码的理由**：查视写回是刻意的（刷新后仍在、第二个读者看得到），而 K-20 已按数据否掉自适应静默期。局限：仍是 jsdom 的计数证据，未在浏览器里量整篇写回的真机耗时 |
| 2026-09-24 | KU-32 一次拖拽悬停只重画目标列 | （本提交） | 基线（先量后改）：新增 `kanban-board-column-memo.test.ts` 7 例，用**非** `memo` 的包装器数列头自己的 render 次数（包在 `memo` 里只会数「列有没有往孩子传新 props」，量不到列有没有重画），在 HEAD 上实跑 3 红——悬停一列时三列全重画（`expected [ 'todo', 'doing', 'done' ] to deeply equal [ 'doing' ]`），拾起一张卡同样全重画。**计划的归因只对一半**：`dnd` bundle 的成员（两个 setter、四种 start/drop 处理器、`handleDragEnd`）本来就各有身份，真凶是 bundle 连同每 render 新建的 `cell` 对象被 `{...props}` 一起透给 memo 的列，比较面上还有四个写入者：`group`（`groupKanbanItems` 每 render 重算）、`selectAll`（`useColumnSelectAll` 每 render 新建）、`onCollapseColumn`（内联箭头）、`toggleCollapse`（`useCollapsedColumns` 里每 render 新建）。修法：`ExpandedBoardColumn` 不再把 `cell`/`dnd` 传下去（cell 在 `useColumnCellHandlers` 里本就按两个键重建），`layout`/`groups`/`bands` 分别按 `[data, view]`/`[data.items, layout]` 记忆，`selectAll` 记忆，`toggleCollapse` 改 `useCallback`，折叠改成收稳定的 `onCollapse` 再以 `group.groupKey` 调用。**变异自检 6 次**：① 把 bundle 退回裸对象→7 例仍全绿（故 `kanban-board-dnd.ts` 未改，少改一个文件）；②–⑥ 重新透传 `{...props}` / 去掉 `groups` 记忆 / 去掉 `selectAll` 记忆 / 折叠退回内联箭头 / `toggleCollapse` 退回普通函数，**各杀 3 例**（均为「悬停只重画目标列」那组），文件按 `/tmp` 备份字节还原。验证：`typecheck` ✅；kanban 93 文件 **1196** 例 ✅；12 项静态门禁 ✅（白名单重算 1015 文件 / 7879 条）。局限：只量了列自己的 render 次数（jsdom），未在浏览器里量真实耗时；`KanbanBoardSwimlanes` 里的 cell 对象仍每次都新建（同一修法下它同样不进比较面），无断言覆盖；卡片层的 memo 仍由 K-19 的 `kanban-repaint-scope.test.ts` 守着，本条只覆盖列层 |
| 2026-09-24 | KU-31 头部不再每次重画都重扫全板标签 | （本提交） | 根因不是扫描的规模而是**时机**：`kanbanActiveItems(data.items)` 写在 `KanbanHeader` 的 render 体内，于是每次重画都交出一只新数组——正是 `KanbanTagFilterBar` 的 `countTags` memo 的键，标签条因此为一次语言切换、一次拖拽高亮落地、任何一次提交把整块板再走一遍（`useMemo(..., [data.items])`，与 F-14 已确立的「聚合读数吃整块板但只算一次」同口径）。新增 `kanban-header-tag-scan.test.ts` 3 例：用 `memo` 包住真标签条数它的渲染次数（与 `kanban-repaint-scope.test.ts` 数卡片同法），断言「动了没卡的重画不重扫」「板子真变了要重扫」「首画只扫一次」。变异自检：去掉 `useMemo`、把 `activeItems` 改回 render 体里的裸调用 → 只有第一条具名例红，`/tmp` 字节还原。**首跑两红都是我自己测试的错**：① `headerProps()` 每次调用现造 `filters`/`sorts`/`selectedTags` 三只空数组，那就是同一棵树里别的 memo 子项的键，被改成模块级常量——不改的话这条用例量的不是 `activeItems`；② 芯片选择器 `[data-kanban-tag]` 不存在（标签芯片是普通 `button`，按文本找），那条断言本就无效，连同一条不成立的「切语言」用例一起删掉（语言重画由 `kanban-memo-locale.test.ts` 覆盖）。`people` 复核结论：`useMemo(() => kanbanPeopleDirectory(data.columns, data.items), [data])` 已按 `data` 记忆，而 `data` 就是它的真实依赖（新卡/新列都可能带来新的人），再收紧反而错，故本条只改一处。验证：`typecheck` ✅（首跑报出我删用例后遗留的 `act` 未使用，已修）；kanban 92 文件 **1189** 例 ✅；12 项静态门禁 ✅。局限：只有 jsdom 的渲染计数证据，未在浏览器里量真实耗时——`countTags` 的成本线性于板子大小，本条消灭的是「不必要的重算次数」而非单次成本；记忆化留在 render 体内而不是抽成 Hook，是因为该文件 494 行、距 `size:check` 的 500 行上限只剩 6 行：抽出的 Hook 连注释要 9 行（实跑 502 行被门禁拦下），故把理由并进那个位置上本就有的那条注释，净增 3 行（497）|
| 2026-09-24 | KU-42 + KU-43 两条安全前提钉成门禁 | （本提交） | 两条同类守卫（都是把既有前提变成会红的测试），合并为一个提交，理由：两者共享同一个 `scripts/check-comments.mjs` 白名单文件，而 `comments:check` 按**工作区**扫描——拆成两个提交时中间快照会出现「白名单有 N 条但树里只有 M 条」的矛盾，一个提交一个自洽快照更符合门禁语义。**KU-42**：`kanban-description-preview.test.ts` 新增敌意用例（`<script>` / `<iframe>` / `<img onerror>` / `javascript:` 链接 / 一行正常 Markdown），断言预览面板里没有 script、没有 iframe、没有任何 `on*` 属性、没有 `javascript:` 链接，且正文照旧渲染成 `<strong>`（否则「把整段描述丢掉」也会让其他断言全绿）。变异自检：把 `renderer/index.ts` 的 `sanitizeProseHtml(raw)` 改回 `raw` → 如实报 `a script tag survived the sanitizer`，`/tmp` 字节还原。**KU-43**：新增 `tests/kanban-url-fields.test.ts`——计划书写的「扫渲染面看有没有过 `safeKanbanUrl`」**与实现不符**：真实护栏在解析边界（`body.ts` 的 `assertFenceUrlsAreSafe`，非法协议把整条栅栏打成错误态），渲染面（`<img src>`、`<a href>`、文本附件的 `fetch`）全部信任已解析的数据，所以「每处渲染都调一次 `safeKanbanUrl`」既非现状也非目标。改成自维护扫描：读 `types.ts` 里每个 `export interface` 的字段名，抽出名字像 URL 的（`url|cover|href|src|thumbnail|image|link|poster|download`），要求结果恰为 `{KanbanFile:['url'], KanbanItem:['cover']}`；再把每个字段逐个喂 `javascript:`/`data:text/html`/协议相对 `//evil…` 断言整条栅栏被拒且错误信息点名字段，并喂三个合法取值（同源路径/跨源 https/`data:image/png`）断言不被误拒。变异自检 2 次：给 `KanbanItem` 加 `thumbnail?: string` → 扫描例红并点名 `thumbnail`；删 `body.ts` 的 files 守卫 → 行为例红并报 `files[].url accepted javascript:alert(1)`，均字节还原。验证：`typecheck` ✅；12 项静态门禁 ✅（`size:check` 先拦下描述预览用例的函数体超 50 行，拆出 `previewDescription()` 夹子后过，未 resnapshot）；两个新文件 7 例全绿；全量 `test:unit` 452 文件 **4098** 例 ✅。局限：KU-42 只涵盖描述预览这一个 `dangerouslySetInnerHTML` 面（看板全树就这一个），其余富文本块走的是同一渲染管线、由各自的既有测试覆盖；KU-43 的扫描是文本层（读 `types.ts`）——字段名不像 URL 时（如 `art?`）扫不到，靠行为例的那张表补 |
| 2026-09-24 | KU-41 文本附件预览的跨源读走外部资源策略 | （本提交） | 基线：`kanban-file-preview-modal.tsx` 的 `useKanbanTextRead` 直接 `fetch(file.url)`——看板里唯一一条不走 `preview.externalImages` 的跨源请求（图片/封面都走 `useKanbanImageAllowed`），且无截止时间、无取消。修法：① `PreviewContent` 把图片与文本两类都交给同一个策略判定（PDF 有意除外：它不请求任何东西，只出一个链接，点下去是读者自己的行为）；② `TextFilePreview({ url, allowed })` 在 `allowed` 为假时**不渲染持有 effect 的组件**，改画新增的 `KanbanBlockedFile`——按构造就没有请求可取消，也不依赖后人记得传标志；③ `useKanbanTextRead` 加 `AbortController` + `KANBAN_FILE_READ_TIMEOUT_MS = 10s`，到期 abort 走同一个 `catch` 落到既有失败态与重试按钮，卸载与重试各自取消上一次读。新增双语键 `preview.kanban_external_file_blocked`（英文写清「读取会联系对方服务器」），新 6 例：策略关时不发出请求且出提示、策略开后正常读、同源附件不受开关影响（守住不过度拦截）、到期给重试、关面板取消、重试取代旧读。变异自检 3 次全杀且字节还原：① `allowed` 改传真值 → 策略例红；② cleanup 不再 abort → 取消两例红；③ 截止时间不再 abort → 到期例红。验证：`typecheck` ✅；11 项静态门禁 ✅；全量 `test:unit` 452 文件 **4093** 例 ✅。局限：浏览器门禁从未打开过附件预览（`e2e-visual.mjs` 里 `kanban-file` 零命中），故本条只有单测证据，未另加浏览器断言（需给夹具造一个跨源附件并监听请求，另项）；同源「文本」附件仍按扩展名/MIME 白名单判定，不读响应头 （本提交） | `commitData` 增第二参数 `kind: 'edit' \| 'view'`（`CommitKanbanData`），`historyReducer` 对 `view` **就地替换**最新状态：照旧写进文档（刷新后仍在、第二个读者也看得到），只是不压步、不清 redo。**规则写成两半**（定义在 `kanban-history.ts` 的 `KanbanCommitKind`）：① 查视＝**既有视图**看起来什么样（搜索、筛选、标签、排序、卡尺寸、隐藏列、卡片字段、分组、泳道、图表类型）与 `activeViewId`；② 编辑＝卡片/子任务/评论/列与选项/板名，以及**哪些视图存在**（建/改名/复制/删除/移动）。分界按「谁在改」而非按字段——以后给视图加字段不必再判一次。落点：`kanban-view-state.ts` 的 toggles 与 `updateActiveView`、`kanban-root-hooks.ts` 的 `setActiveViewId`、以及**审计时揪出的漏网者** `kanban-column-hooks.ts` 的 `handleChangeGroupBy`（它直写 `views[].groupBy`，与走 `updateActiveView` 的泳道选择是同一种选择，原先两处会分到不同 kind）。**代价已在代码注释里写明**：撤销恢复的是整份文档状态，所以最后一次编辑之后做的查视会跟着回退（要保住它就得把每份快照的视图那一半单独存并回放，即第二套历史模型）。新增 `kanban-history.test.ts` 9 例（文档写入/查视不压步/连点后撤销回到上一步编辑/redo 仍回到最新查视/无变化不占步/交错顺序）+ 新文件 `kanban-lookup-history.test.ts` 4 例（端到端：用根状态上**控件真正调用的那些 writer** 走 13 次查视，撤销回到上次卡片编辑、第二次撤销回到更早那次、每次查视都照旧落文档、查视之间删视图仍保留步数）——为守 500 行/单函数 50 行硬门禁把与「视图持久化」无关的那组从 `kanban-view-state.test.ts` 拆出（该文件回 420 行）。变异自检 4 次全杀且 `/tmp` 备份字节还原：① reducer 去掉 `view` 分支 → 历史 3 例 + 端到端 2 例红；② 写者不再声明 kind → 端到端 2 例红（历史单测不红，正是端到端存在的理由）；③ `setActiveViewId` 退回默认 → 端到端 2 例红；④ `handleChangeGroupBy` 退回默认 → 端到端 2 例红。**浏览器门禁新增 `assertKanbanLookupUndo` 5 条**（真写一张卡 → 真按「逾期」芯片把它筛掉 → 真按面板上的撤销：断言撤销没有退回那次查视，且交回的是读者写的那张卡而不是那颗芯片）；首跑即抓出我自己的两处错断言（`readKanbanQuickState` 的 `keys` 是 item id 不是标题——按标题改判后又发现「芯片把板压到 1 张」不等于「张数不变」，改为逐条读标题 + 张数变少），修完实跑 524/0；变异（reducer 退回）实跑 8 红，其中新增那 4 条如实点名（另 4 条是那张没被撤销掉的卡留下的旁证）。局限：非共享令牌与配色未动故未跑 `contrast:check`（无绘制面变化）；浏览器只在全屏覆盖层走（笔记里撤销控件收在 ⋯ 溢出菜单里，是另一条打开路径，由参考卡场景覆盖）；查视之间「最后一次编辑之后做的查视会被撤销一并带回」是既定代价；`updateActiveView` 与 `handleChangeGroupBy` 是 `views[].groupBy` 的两个写者（本轮只统一了 kind，未合并写法，登记为后续） |
