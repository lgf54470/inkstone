...
| 2026-09-18 | K1-03 视图状态按 viewId 持久化 + 死字段接线/删除 | 本次提交（hash 由下一次提交回填） | 新增 kanban-view-state.test.ts 4 例 + tests/kanban-view-live-fields.test.ts 1 例（先红后绿）：filters/sorts/searchQuery/cardSize 从组件内存 useState 改为读写 activeView（updateActiveView 经 commitData 落盘），切视图/关全屏/刷新不再丢配置，各视图互不串味；activeViewId 选择即写回 data.activeViewId；未配置时 filters/sorts 复用模块常量保持数组身份稳定。7 死字段处置：dateField/startField/endField/progressField 接线（timeline-helpers 收 TimelineDateFields 按配置字段取数、配置缺失回退 startDate/dueDate 旧链；calendar-helpers.getWeekEventSegments 收 dateField 且保留多日跨度语义；甘特进度读写走 progressField，日历日新建也写配置字段），coverField/chartMetric/hiddenProperties 无人读且无近期接线计划——按「不留死字段」从 KanbanView 删除（历史栅栏多余键 parse 本就忽略）。live-fields 扫描守卫：KanbanView 每个字段必须在 types.ts 之外有 `.field` 读取方，新配置旋钮不接线即被拒。全量 test:unit 227 文件 1744 测试 ✅，typecheck/11 项静态门禁 ✅ |
