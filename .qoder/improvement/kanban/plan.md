...
| 2026-09-18 | K1-07c 板上文字改走填充对比令牌 + text-white 扫描门禁 | 本次提交（hash 由下一次提交回填） | 新增 tests/kanban-contrast-tokens.test.ts 3 例（先红后绿，红时列出全部 9 处违规）：模块源码不得再出现 `text-white`/`text-black`/`text-[#hex]`；`--accent-contrast` 对 7 个强调色 × 两主题 ≥4.5:1（实测浅 6.56–9.70、深 7.22–10.42）；`--accent-contrast` 对 12 个标签前景色 ≥4.5:1（浅 6.21–6.77、深 7.86–8.13，同一处白色在暗色标签上实测仅 2.39–2.47）。落地：日历/时间轴/甘特「今天」徽标、日期选择器选中日、表格头像、列表完成勾选、子任务勾选、附件预览按钮、配色选择器勾号共 9 处 `text-white` → `text-[var(--accent-contrast)]`（app 既有「填充上文字」约定，kanban-header/date-range-popover 等已在用）；时间轴与甘特的「今天」由 `bg-[var(--danger)]`（白字实测浅 3.69、深 3.14，两主题都不达标且该目录再无其他 danger 用法）改为与日历一致的强调填充；配色选择器勾号顺带删掉为救可读性加的 drop-shadow。theme-tokens.mjs 增 `accentNames`/`accentVars`，按两属性选择器压过其后 `:root` 默认值的级联取该强调色。全量 test:unit 234 文件 1787 测试 ✅，typecheck/11 项静态门禁 ✅ |
