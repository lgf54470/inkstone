...
| 2026-09-18 | K-17 栅栏 URL 协议白名单 | 本次提交（hash 由下一次提交回填） | 新增 url.ts safeKanbanUrl()（放行 /api 相对路径、http(s)、blob:、data:image/*；拒 javascript:/file:/data:text、// 协议相对、控制符混淆，共 4 例 url.test.ts 先红后绿）+ body.test.ts 3 例：normalizeKanbanData 经 assertFenceUrlsAreSafe 清洗 item.cover 与 files[].url，非法协议整体走错误态提示不静默丢字段。UI 侧未逐处过滤：normalize 是唯一数据入口（上传回包为自家 /api 相对 URL），与 review #11 所列 3 个 UI 文件收敛为此单点（默认渲染数据均已过闸）。顺带把默认列/视图提为 defaultKanbanColumns()/defaultKanbanViews() 以满足 50 行函数上限。kanban+preview 32 文件 169 ✅，全量 test:unit 219 文件 1699 测试 ✅，typecheck/11 项静态门禁 ✅ |
