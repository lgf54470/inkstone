...
async function seedVisit(db: D1Shim, slug: string, visitedAt: number, userId = OWNER): Promise<void> {
  await runSql(
    db,
    `INSERT INTO shares (slug, note_id, user_id, tags, is_enabled, created_at)
     VALUES (?1, ?2, ?3, '[]', 1, ?4)`,
    slug, `note-${slug}`, userId, NOW,
  )
  await runSql(
    db,
    `INSERT INTO share_visits (user_id, note_id, slug, visited_at, visitor_fp, country, device_type, os, browser)
     VALUES (?1, ?2, ?3, ?4, ?5, 'US', 'desktop', 'macOS', 'Safari')`,
    userId, `note-${slug}`, slug, visitedAt, `fp-${slug}`,
  )
}

async function survivingSlugs(db: D1Shim): Promise<string[]> {
  const rows = await allRows(db, 'SELECT slug FROM share_visits ORDER BY visited_at, slug')
  return rows.map((row) => String(row.slug))
}

describe('share visit log retention sweep (SH-05c)', () => {
  it('deletes rows past the retention the account stored and keeps newer ones', async () => {
    const db = await makeDb()
    await seedUser(db, OWNER, { share: { visitLogRetentionDays: 7 } })
    ...
    expect(result.shareVisitLogs).toBe(1)
  })

  it('keeps every row for an account that chose unlimited retention', async () => {
    const db = await makeDb()
    await seedUser(db, OWNER, { share: { visitLogRetentionDays: 0 } })
    await seedVisit(db, 'ancient', NOW - 3_650 * DAY_MS)

    const result = await purgeExpiredOperationalData(db as unknown as D1Database, NOW)

    expect(await survivingSlugs(db)).toEqual(['ancient'])
    expect(result.shareVisitLogs).toBe(0)
  })

  it('sweeps at the default when the stored settings carry no share section', async () => {
    const db = await makeDb()
  ...
  it('judges every account by its own retention', async () => {
    const db = await makeDb()
    await seedUser(db, OWNER, { share: { visitLogRetentionDays: 7 } })
    await seedUser(db, OTHER, { share: { visitLogRetentionDays: 365 } })
  ...
  it('leaves a retention value the sweep cannot parse alone rather than deleting everything', async () => {
    const db = await makeDb()
    await seedUser(db, OWNER, { share: { visitLogRetentionDays: 'last week' } })
